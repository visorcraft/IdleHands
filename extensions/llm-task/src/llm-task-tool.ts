...
import Ajv from "ajv";
import { resolvePreferredIdleHandsTmpDir } from "idlehands/plugin-sdk";
// NOTE: This extension is intended to be bundled with IdleHands.
// When running from source (tests/dev), IdleHands internals live under src/.
...
export function createLlmTaskTool(api: IdleHandsPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredIdleHandsTmpDir(), "idlehands-llm-task-"),
        );
