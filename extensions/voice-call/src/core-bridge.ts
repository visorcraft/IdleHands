...
function resolveIdleHandsRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.IDLEHANDS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set IDLEHANDS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveIdleHandsRoot(), "dist", "extensionAPI.js");
