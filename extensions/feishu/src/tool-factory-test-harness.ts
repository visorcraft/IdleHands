import type { AnyAgentTool, IdleHandsPluginApi } from "idlehands/plugin-sdk";
...
export function createToolFactoryHarness(cfg: IdleHandsPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<IdleHandsPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
