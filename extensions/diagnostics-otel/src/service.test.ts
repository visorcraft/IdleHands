...
vi.mock("idlehands/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("idlehands/plugin-sdk")>("idlehands/plugin-sdk");
  return {
    ...actual,
...
import type { IdleHandsPluginServiceContext } from "idlehands/plugin-sdk";
import { emitDiagnosticEvent } from "idlehands/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/idlehands-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): IdleHandsPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("idlehands.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("idlehands.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("idlehands.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("idlehands.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("idlehands.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("idlehands.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("idlehands.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("idlehands.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("idlehands.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("idlehands.webhook.processed");
    expect(spanNames).toContain("idlehands.message.processed");
    expect(spanNames).toContain("idlehands.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["idlehands.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("idlehands.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "idlehands.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["idlehands.reason"]).toBe("string");
    expect(String(attrs?.["idlehands.reason"])).not.toContain(
