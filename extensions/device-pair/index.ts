...
function resolveGatewayPort(cfg: IdleHandsPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.IDLEHANDS_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: IdleHandsPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: IdleHandsPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the IdleHands iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the IdleHands iOS app:",
            "",
            "```",
            qrAscii,
            "```",
