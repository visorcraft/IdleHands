...
import type { ReplyPayload, IdleHandsConfig } from "idlehands/plugin-sdk";
import { createReplyPrefixOptions } from "idlehands/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // IdleHandsConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as IdleHandsConfig;
