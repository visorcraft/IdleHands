/**
 * Twitch channel plugin for IdleHands.
 *
...
import { buildChannelConfigSchema } from "idlehands/plugin-sdk";
 ...
 * for IdleHands. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: IdleHandsConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: IdleHandsConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: IdleHandsConfig): boolean => {
