...
IdleHands extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
idlehands channels login --channel zalouser
...
```bash
idlehands message send --channel zalouser --target <threadId> --message "Hello from IdleHands!"
```
...
```bash
idlehands channels login --channel zalouser              # Login via QR
idlehands channels login --channel zalouser --account work
idlehands channels status --probe
idlehands channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
idlehands directory self --channel zalouser
idlehands directory peers list --channel zalouser --query "name"
idlehands directory groups list --channel zalouser --query "work"
idlehands directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
idlehands message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
idlehands message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `idlehands channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
idlehands channels login --channel zalouser --account work
idlehands message send --channel zalouser --account work --target <id> --message "Hello"
