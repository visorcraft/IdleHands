...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.IDLEHANDS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-nostr-"));
  process.env.IDLEHANDS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.IDLEHANDS_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".idlehands");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.IDLEHANDS_STATE_DIR = previous;
    }
