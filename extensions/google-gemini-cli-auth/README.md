# Google Gemini CLI Auth (IdleHands plugin)
...
```bash
idlehands plugins enable google-gemini-cli-auth
```
...
```bash
idlehands models auth login --provider google-gemini-cli --set-default
```
...
- `IDLEHANDS_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `IDLEHANDS_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
