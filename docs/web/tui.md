...
```bash
idlehands tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `idlehands tui --deliver`
...
- Check the Gateway logs: `idlehands logs --follow`.
- Confirm the agent can run: `idlehands status` and `idlehands models status`.
...
- No agents in picker: check `idlehands agents list` and your routing config.
