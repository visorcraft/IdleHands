...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/idlehands`)
...
If the page fails to load, start the Gateway first: `idlehands gateway`.
...
you revoke it with `idlehands devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.idlehands/idlehands.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop idlehands`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. IdleHands
...
IdleHands **blocks** Control UI connections without device identity.
