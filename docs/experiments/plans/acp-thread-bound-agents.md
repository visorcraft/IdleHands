...
This plan defines how IdleHands should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- IdleHands core owns ACP control plane concerns
...
IdleHands should not reimplement ACP transport internals in core.
IdleHands should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in IdleHands, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under IdleHands state dir
...
Core (must be in IdleHands core):
...
- ACP event stream is normalized to IdleHands reply chunks
...
For the first production backend (`extensions/acpx`), IdleHands and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses IdleHands `sessionKey`)
...
- IdleHands consumes ndjson events from `acpx --format json --json-strict`
