...
read_when:
  - Designing workspace memory (~/.idlehands/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep IdleHands integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.idlehands/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.idlehands/workspace` as canonical human-readable memory.
...
```
~/.idlehands/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in IdleHands**, but keep a separable core library.

### Why integrate into IdleHands?

- IdleHands already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `idlehands memory recall "…" --k 25 --since 30d`
  - `idlehands memory reflect --since 7d`
...
Pragmatic take for `~/.idlehands/workspace`:
