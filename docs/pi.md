...
summary: "Architecture of IdleHands's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in IdleHands
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how IdleHands integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

IdleHands uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, IdleHands directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in IdleHands's local TUI mode)                                             |
...
├── pi-tools.ts                    # createIdleHandsCodingTools()
...
├── idlehands-tools.ts              # IdleHands-specific tools
...
Image injection is prompt-local: IdleHands loads image refs from the current prompt and
...
2. **Custom Replacements**: IdleHands replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **IdleHands Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures IdleHands's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, IdleHands CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
IdleHands wraps this with `guardSessionManager()` for tool result safety.
...
IdleHands maintains an auth profile store with multiple API keys per provider:
...
IdleHands loads custom pi extensions for specialized behavior:
...
IdleHands also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | IdleHands Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom IdleHands tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.idlehands/agents/<agentId>/sessions/` (or `$IDLEHANDS_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `IDLEHANDS_LIVE_TEST=1`)
