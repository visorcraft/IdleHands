---
summary: "CLI reference for `idlehands onboard` (interactive onboarding wizard)"
read_when:
...
```bash
idlehands onboard
idlehands onboard --flow quickstart
idlehands onboard --flow manual
idlehands onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `idlehands dashboard` (Control UI, no channel setup).
