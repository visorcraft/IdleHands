---
summary: "CLI reference for `idlehands qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
idlehands qr
idlehands qr --setup-code-only
idlehands qr --json
idlehands qr --remote
idlehands qr --url wss://gateway.example/ws --token '<token>'
```
