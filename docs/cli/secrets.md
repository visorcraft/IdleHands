---
summary: "CLI reference for `idlehands secrets` (reload, audit, configure, apply)"
read_when:
...
Use `idlehands secrets` to migrate credentials from plaintext to SecretRefs and keep the active secrets runtime healthy.
...
```bash
idlehands secrets audit
idlehands secrets audit --check
idlehands secrets audit --json
```
...
```bash
idlehands secrets configure
idlehands secrets configure --plan-out /tmp/idlehands-secrets-plan.json
idlehands secrets configure --apply --yes
idlehands secrets configure --providers-only
idlehands secrets configure --skip-provider-setup
idlehands secrets configure --json
```
...
- `configure` targets secret-bearing fields in `idlehands.json`.
...
idlehands secrets apply --from /tmp/idlehands-secrets-plan.json
idlehands secrets apply --from /tmp/idlehands-secrets-plan.json --dry-run
idlehands secrets apply --from /tmp/idlehands-secrets-plan.json --json
```
...
- `idlehands.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.idlehands/.env` known secret keys whose values were migrated
