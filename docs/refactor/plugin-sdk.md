...
- Publish as `idlehands/plugin-sdk` (or export from core under `idlehands/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `IdleHandsPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: IdleHandsConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: IdleHandsConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: IdleHandsConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: IdleHandsConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: IdleHandsConfig): string;
  };
};
```
...
- Introduce `idlehands/plugin-sdk`.
- Add `api.runtime` to `IdleHandsPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `idlehandsRuntime: ">=2026.2.0"`).
