...
IdleHands supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, IdleHands performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). IdleHands validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/IdleHands QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/idlehands"],
...
When reload-time activation fails after a healthy state, IdleHands enters degraded secrets state.
...
```bash
idlehands secrets audit --check
idlehands secrets configure
idlehands secrets audit --check
```
...
- plaintext values at rest (`idlehands.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `idlehands.json`
...
- `idlehands secrets configure --providers-only`
- `idlehands secrets configure --skip-provider-setup`
...
IdleHands intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, IdleHands runtime no longer depends on plaintext `auth.json`.
