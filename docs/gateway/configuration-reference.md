...
description: "Complete field-by-field reference for ~/.idlehands/idlehands.json"
summary: "Complete reference for every IdleHands config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.idlehands/idlehands.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — IdleHands uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `idlehands doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["idlehands-dm"] },
      guilds: {
        "123456789012345678": {
...
- IdleHands additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `idlehands plugins install @idlehands/mattermost`.
...
IdleHands spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `idlehands channels add` (or channel onboarding) while still on a single-account top-level channel config, IdleHands moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `idlehands doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@idlehands"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `idlehands.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, IdleHands auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, IdleHands assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "idlehands-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.idlehands/sandboxes`
...
noVNC observer access uses VNC auth by default and IdleHands emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `idlehands-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.idlehands/agents/main/agent",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, IdleHands starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:idlehands`   | All built-in tools (excludes provider plugins)                                           |
...
IdleHands uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.idlehands/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `IDLEHANDS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `idlehands onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `idlehands onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `idlehands onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `idlehands onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `idlehands onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `idlehands onboard --auth-choice minimax-api`.
...
- Loaded from `~/.idlehands/extensions`, `<workspace>/.idlehands/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `idlehands plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or IDLEHANDS_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `IDLEHANDS_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.idlehands-dev` + port `19001`), `--profile <name>` (uses `~/.idlehands-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.idlehands/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-idlehands-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `IDLEHANDS_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or IDLEHANDS_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__idlehands__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__idlehands__/a2ui/`
...
- Also serves A2UI at `/__idlehands__/a2ui/`.
...
- Hostname defaults to `idlehands`. Override with `IDLEHANDS_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.idlehands/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `idlehands dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.idlehands/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${IDLEHANDS_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.idlehands/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/idlehands-vault-resolver",
...
- Legacy OAuth imports from `~/.idlehands/credentials/oauth.json`.
...
- Default log file: `/tmp/idlehands/idlehands-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `idlehands doctor --fix` can strip unknown keys).
