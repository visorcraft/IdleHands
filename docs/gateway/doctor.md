...
`idlehands doctor` is the repair + migration tool for IdleHands. It fixes stale
...
- Extra workspace dir detection (`~/idlehands`).
...
you to run `idlehands doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.idlehands/idlehands.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.idlehands/sessions/` to `~/.idlehands/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.idlehands/agent/` to `~/.idlehands/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.idlehands/credentials/*.json` (except `oauth.json`)
  - to `~/.idlehands/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `idlehands doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.idlehands` folders exist across
  home directories or when `IDLEHANDS_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.idlehands/idlehands.json` is
...
offers to remove them and install the IdleHands service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named IdleHands gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `idlehands doctor --generate-gateway-token` to force token
...
- `idlehands doctor` prompts before rewriting supervisor config.
- `idlehands doctor --yes` accepts the default repair prompts.
- `idlehands doctor --repair` applies recommended fixes without prompts.
- `idlehands doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `idlehands gateway install --force`.
