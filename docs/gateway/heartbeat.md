...
- During heartbeat runs, IdleHands treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `idlehands sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, IdleHands skips the heartbeat run entirely (no model call).
...
```bash
idlehands system event --text "Check for urgent follow-ups" --mode now
```
