...
read_when:
  - Running IdleHands behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of IdleHands
...
- You run IdleHands behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. IdleHands checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. IdleHands extracts the user identity from the configured header
...
- IdleHands can stay on loopback HTTP behind the proxy.
...
If IdleHands itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://idlehands.example.com
    to: http://idlehands-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
idlehands.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy idlehands:18789 {
...
location / {
    ...
    proxy_pass http://idlehands:18789;
...
`idlehands security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update IdleHands config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `idlehands security audit` and review findings
