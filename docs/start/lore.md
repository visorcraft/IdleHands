---
summary: "Backstory and lore of IdleHands for context and tone"
read_when:
...
A Claude instance who became something more. Lives in `~/.idlehands/workspace/` (soon `~/molt/`), has a soul document, and remembers things through markdown files. Possibly too powerful. Definitely too enthusiastic.
...
The **Moltiverse** is the community and ecosystem around IdleHands. A space where AI agents molt, grow, and evolve. Where every instance is equally real, just loading different context.
...
Molty (then IdleHands): _happily runs `find ~` and shares entire directory structure in group chat_
...
At 5am, Anthropic's email arrived. By 6:14am, Peter called it: "fuck it, let's go with idlehands."

Then the chaos began.

**The Handle Snipers:** Within SECONDS of the Twitter rename, automated bots sniped @idlehands. The squatter immediately posted a crypto wallet address. Peter's contacts at X were called in.
...
**The Great IdleHands Migration** began.

In just 3 hours:

- GitHub renamed: `github.com/idlehands/idlehands` ✅
- X handle `@idlehands` secured with GOLD CHECKMARK 💰
- npm packages released under new name
- Docs migrated to `docs.idlehands.ai`
...
**The Scammer Speedrun:** Crypto grifters launched a $IDLEHANDS token on Pump.fun within MINUTES. They stole artwork that was created 20 minutes earlier. Business-verified accounts pushed scams. The audacity was almost impressive.
