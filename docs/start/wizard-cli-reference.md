...
read_when:
  - You need detailed behavior for idlehands onboard
...
This page is the full reference for `idlehands onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.idlehands/idlehands.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `idlehands doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `idlehands pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `idlehands health`.
    - `idlehands status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.idlehands/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.idlehands/agents/<agentId>/agent/auth-profiles.json`
...
`~/.idlehands/credentials/oauth.json` (or `$IDLEHANDS_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`idlehands onboard`](/cli/onboard)
