---
summary: "IdleHands on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up IdleHands on a Raspberry Pi
  - Running IdleHands on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# IdleHands on Raspberry Pi

## Goal

Run a persistent, always-on IdleHands Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most IdleHands features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled idlehands

# Enable if not
sudo systemctl enable idlehands

# Start on boot
sudo systemctl start idlehands
```
...
```bash
# Check logs
journalctl -u idlehands --no-pager -n 100

# Common fix: rebuild
cd ~/idlehands  # if using hackable install
npm run build
sudo systemctl restart idlehands
```
