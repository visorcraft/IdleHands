...
service via the external `idlehands` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.idlehands.gateway`
  (or `ai.idlehands.<profile>` when using `--profile`/`IDLEHANDS_PROFILE`; legacy `com.idlehands.*` is supported).
