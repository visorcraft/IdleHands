...
- Same path: run the app from a fixed location (for IdleHands, `dist/IdleHands.app`).
...
```bash
sudo tccutil reset Accessibility ai.idlehands.mac
sudo tccutil reset ScreenCapture ai.idlehands.mac
...
Workaround: move files into the IdleHands workspace (`~/.idlehands/workspace`) if you want to avoid per-folder grants.
