...
- `/usage cost` in chats: local cost summary aggregated from IdleHands session logs.
- CLI: `idlehands status --usage` prints a full per-provider breakdown.
- CLI: `idlehands channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
