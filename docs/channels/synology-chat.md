---
summary: "Synology Chat webhook setup and IdleHands config"
read_when:
  - Setting up Synology Chat with IdleHands
...
```bash
idlehands plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your IdleHands gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in IdleHands.
...
- Pairing approvals work with:
  - `idlehands pairing list synology-chat`
  - `idlehands pairing approve synology-chat <CODE>`
...
```bash
idlehands message send --channel synology-chat --target 123456 --text "Hello from IdleHands"
idlehands message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
