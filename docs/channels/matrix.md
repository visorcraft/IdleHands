...
Matrix is an open, decentralized messaging protocol. IdleHands connects as a Matrix **user**
...
```bash
idlehands plugins install @idlehands/matrix
```

Local checkout (when running from a git repo):

```bash
idlehands plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `idlehands plugins install @idlehands/matrix`
   - From a local checkout: `idlehands plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: IdleHands calls the same
     login endpoint, stores the access token in `~/.idlehands/credentials/matrix/credentials.json`,
...
- On first connection, IdleHands requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  IdleHands logs a warning.
...
`~/.idlehands/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `idlehands pairing list matrix`
  - `idlehands pairing approve matrix <CODE>`
...
- On startup, IdleHands resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
