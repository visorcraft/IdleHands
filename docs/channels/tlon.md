...
Tlon is a decentralized messenger built on Urbit. IdleHands connects to your Urbit ship and can
...
```bash
idlehands plugins install @idlehands/tlon
```

Local checkout (when running from a git repo):

```bash
idlehands plugins install ./extensions/tlon
```
...
By default, IdleHands blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `idlehands message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, IdleHands replies in-thread.
