---
summary: "Mattermost bot setup and IdleHands config"
read_when:
...
```bash
idlehands plugins install @idlehands/mattermost
```

Local checkout (when running from a git repo):

```bash
idlehands plugins install ./extensions/mattermost
```
...
4. Configure IdleHands and start the gateway.
...
- Approve via:
  - `idlehands pairing list mattermost`
  - `idlehands pairing approve mattermost <CODE>`
...
Use these target formats with `idlehands message send` or cron/webhooks:
