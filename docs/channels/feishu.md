...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects IdleHands to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
idlehands plugins install @idlehands/feishu
```

Local checkout (when running from a git repo):

```bash
idlehands plugins install ./extensions/feishu
```
...
If you just installed IdleHands, run the wizard:
...
2. Configuring app credentials in IdleHands
...
1. You already ran `idlehands channels add` for Feishu
2. The gateway is running (`idlehands gateway status`)
...
```bash
idlehands pairing approve feishu <CODE>
```
...
2. Run `idlehands logs --follow` and look for `chat_id`
...
2. Run `idlehands logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `idlehands gateway status`  | Show gateway status           |
| `idlehands gateway install` | Install/start gateway service |
| `idlehands gateway stop`    | Stop gateway service          |
| `idlehands gateway restart` | Restart gateway service       |
| `idlehands logs --follow`   | Tail gateway logs             |
...
4. Check logs: `idlehands logs --follow`
...
5. Ensure the gateway is running: `idlehands gateway status`
6. Check logs: `idlehands logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.idlehands/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.idlehands/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
