...
read_when:
  - Setting up Twitch chat integration for IdleHands
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. IdleHands connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
idlehands plugins install @idlehands/twitch
```

Local checkout (when running from a git repo):

```bash
idlehands plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `IDLEHANDS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "idlehands", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use IDLEHANDS_TWITCH_ACCESS_TOKEN env var)
...
```bash
IDLEHANDS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
