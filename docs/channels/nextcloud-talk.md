...
```bash
idlehands plugins install @idlehands/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
idlehands plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "IdleHands" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `idlehands pairing list nextcloud-talk`
  - `idlehands pairing approve nextcloud-talk <CODE>`
