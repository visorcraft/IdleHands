...
read_when:
  - You want IdleHands to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables IdleHands to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`idlehands onboard`) and `idlehands channels add` list optional channel plugins.
...
```bash
idlehands plugins install @idlehands/nostr
```

Use a local checkout (dev workflows):

```bash
idlehands plugins install --link <path-to-idlehands>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "idlehands",
        "displayName": "IdleHands",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "idlehands@example.com",
        "lud16": "idlehands@example.com"
      }
    }
  }
}
