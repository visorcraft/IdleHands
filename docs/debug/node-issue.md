...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in IdleHands
...
Running IdleHands via Node with `tsx` fails at startup with:

```
[idlehands] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `idlehands status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch idlehands.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node idlehands.mjs status` works on Node 25.
