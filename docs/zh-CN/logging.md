...
你可以在 `~/.idlehands/idlehands.json` 中覆盖此设置：
...
所有日志配置都在 `~/.idlehands/idlehands.json` 的 `logging` 下。
...
- IdleHands 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `idlehands plugins enable diagnostics-otel` 启用插件。
...
- `idlehands.tokens`（计数器，属性：`idlehands.token`、`idlehands.channel`、`idlehands.provider`、`idlehands.model`）
- `idlehands.cost.usd`（计数器，属性：`idlehands.channel`、`idlehands.provider`、`idlehands.model`）
- `idlehands.run.duration_ms`（直方图，属性：`idlehands.channel`、`idlehands.provider`、`idlehands.model`）
- `idlehands.context.tokens`（直方图，属性：`idlehands.context`、`idlehands.channel`、`idlehands.provider`、`idlehands.model`）

消息流：

- `idlehands.webhook.received`（计数器，属性：`idlehands.channel`、`idlehands.webhook`）
- `idlehands.webhook.error`（计数器，属性：`idlehands.channel`、`idlehands.webhook`）
- `idlehands.webhook.duration_ms`（直方图，属性：`idlehands.channel`、`idlehands.webhook`）
- `idlehands.message.queued`（计数器，属性：`idlehands.channel`、`idlehands.source`）
- `idlehands.message.processed`（计数器，属性：`idlehands.channel`、`idlehands.outcome`）
- `idlehands.message.duration_ms`（直方图，属性：`idlehands.channel`、`idlehands.outcome`）

队列 + 会话：

- `idlehands.queue.lane.enqueue`（计数器，属性：`idlehands.lane`）
- `idlehands.queue.lane.dequeue`（计数器，属性：`idlehands.lane`）
- `idlehands.queue.depth`（直方图，属性：`idlehands.lane` 或 `idlehands.channel=heartbeat`）
- `idlehands.queue.wait_ms`（直方图，属性：`idlehands.lane`）
- `idlehands.session.state`（计数器，属性：`idlehands.state`、`idlehands.reason`）
- `idlehands.session.stuck`（计数器，属性：`idlehands.state`）
- `idlehands.session.stuck_age_ms`（直方图，属性：`idlehands.state`）
- `idlehands.run.attempt`（计数器，属性：`idlehands.attempt`）
...
- `idlehands.webhook.error`
  - `idlehands.channel`、`idlehands.webhook`、`idlehands.chatId`、`idlehands.error`
- `idlehands.message.processed`
  - `idlehands.channel`、`idlehands.outcome`、`idlehands.chatId`、`idlehands.messageId`、`idlehands.sessionKey`、`idlehands.sessionId`、`idlehands.reason`
- `idlehands.session.stuck`
  - `idlehands.state`、`idlehands.ageMs`、`idlehands.queueDepth`、`idlehands.sessionKey`、`idlehands.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `idlehands doctor`。
