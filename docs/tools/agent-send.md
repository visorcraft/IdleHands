---
summary: "Direct `idlehands agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `idlehands agent` (direct agent runs)

`idlehands agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `idlehands message --target`).
...
```bash
idlehands agent --to +15555550123 --message "status update"
...
idlehands agent --to +15555550123 --message "Summon reply" --deliver
