---
summary: "Chrome extension: let IdleHands drive your existing Chrome tab"
read_when:
...
The IdleHands Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate idlehands-managed Chrome profile.
...
IdleHands then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the IdleHands release (npm package) as static files. There is no separate “build” step.

After upgrading IdleHands:

- Re-run `idlehands browser extension install` to refresh the installed files under your IdleHands state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

IdleHands ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `IDLEHANDS_GATEWAY_TOKEN`)
...
- Open the tab you want IdleHands to control.
...
- `ON`: attached; IdleHands can drive that tab.
...
Debugging: `idlehands sandbox explain`
...
`idlehands browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `idlehands browser extension install` first to copy the extension to a stable location under your IdleHands state directory.
...
- **This is not isolated** like the dedicated idlehands-managed profile.
