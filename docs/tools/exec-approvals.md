...
- **gateway host** → `idlehands` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.idlehands/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.idlehands/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `idlehands approvals allowlist ...`).
- `idlehands security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `idlehands doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.idlehands/exec-approvals.json` directly.

CLI: `idlehands approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
