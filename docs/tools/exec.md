...
- `gateway`/`node` approvals are controlled by `~/.idlehands/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  IdleHands prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`idlehands security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `idlehands doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
