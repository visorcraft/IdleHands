...
IdleHands ships two lightweight web tools:
...
If no `provider` is explicitly set, IdleHands auto-detects which provider to use based on available API keys, checking in this order:
...
3. Run `idlehands configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `idlehands configure --section web`. It stores the key in
`~/.idlehands/idlehands.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.idlehands/.env` (or your
service environment). See [Env vars](/help/faq#how-does-idlehands-load-environment-variables).
...
environment. For a gateway install, put it in `~/.idlehands/.env`.

If no base URL is set, IdleHands chooses a default based on the API key source:
...
For a gateway install, put it in `~/.idlehands/.env`.
