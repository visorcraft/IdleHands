...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `idlehands sandbox explain`.
...
Legacy `agent.*` configs are migrated by `idlehands doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=idlehands-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${IDLEHANDS_STATE_DIR:-$HOME/.idlehands}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
