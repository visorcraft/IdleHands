---
summary: "How IdleHands builds prompt context and reports token usage + costs"
read_when:
...
IdleHands tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

IdleHands assembles its own system prompt on every run. It includes:
...
For images, IdleHands downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from IdleHands session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `idlehands status --usage` and `idlehands channels list` show
...
`cacheWrite`. If pricing is missing, IdleHands shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. IdleHands can
...
Anthropic's 1M context window is currently beta-gated. IdleHands can inject the
...
IdleHands skips the `context-1m-*` beta header because Anthropic currently
