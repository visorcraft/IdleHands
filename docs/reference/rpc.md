...
IdleHands integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- IdleHands owns lifecycle when `channels.signal.autoStart=true`.
...
- IdleHands spawns `imsg rpc` as a child process (legacy iMessage integration).
