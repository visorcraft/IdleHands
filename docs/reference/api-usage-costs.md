...
IdleHands features that can generate provider usage or paid API calls.
...
- `idlehands status --usage` and `idlehands channels list` show provider **usage windows**
...
IdleHands can pick up credentials from:
...
- `idlehands status --usage`
- `idlehands models status --json`
...
`idlehands models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
