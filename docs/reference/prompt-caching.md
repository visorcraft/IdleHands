...
- With Anthropic API-key auth profiles, IdleHands seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, IdleHands injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
IdleHands exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.idlehands/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$IDLEHANDS_STATE_DIR/logs/cache-trace.jsonl`
...
- `IDLEHANDS_CACHE_TRACE=1` enables cache tracing.
- `IDLEHANDS_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `IDLEHANDS_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `IDLEHANDS_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `IDLEHANDS_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
