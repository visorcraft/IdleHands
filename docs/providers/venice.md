---
summary: "Use Venice AI privacy-focused models in IdleHands"
read_when:
  - You want privacy-focused inference in IdleHands
...
```bash
idlehands onboard --auth-choice venice-api-key
```
...
```bash
idlehands agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, IdleHands shows all available Venice models. Pick based on your needs:
...
```bash
idlehands models set venice/claude-opus-45
idlehands models set venice/llama-3.3-70b
```

List all available models:

```bash
idlehands models list | grep venice
```

## Configure via `idlehands configure`

1. Run `idlehands configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
IdleHands automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
idlehands agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
idlehands agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
idlehands agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
idlehands agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
idlehands agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `idlehands models list` to see currently available models. Some models may be temporarily offline.
