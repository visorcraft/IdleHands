---
summary: "Use Synthetic's Anthropic-compatible API in IdleHands"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. IdleHands registers it as the
...
Note: IdleHands's Anthropic client appends `/v1` to the base URL, so use
