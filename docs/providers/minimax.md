---
summary: "Use MiniMax M2.1 in IdleHands"
read_when:
  - You want MiniMax models in IdleHands
...
```bash
idlehands plugins enable minimax-portal-auth  # skip if already loaded.
idlehands gateway restart  # restart if gateway is already running
idlehands onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/idlehands/idlehands/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `idlehands.json`:
...
- Use `idlehands models list` and `idlehands models set minimax/MiniMax-M2.1` to switch.
...
- Running `idlehands configure` and selecting **MiniMax M2.1**, or
