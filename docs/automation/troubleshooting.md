...
idlehands system heartbeat last
idlehands logs --follow
idlehands config get agents.defaults.heartbeat
idlehands channels status --probe
```
...
```bash
idlehands config get agents.defaults.heartbeat.activeHours
idlehands config get agents.defaults.heartbeat.activeHours.timezone
idlehands config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
idlehands cron list
idlehands logs --follow
```
