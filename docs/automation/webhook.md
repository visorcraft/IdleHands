...
read_when:
  ...
  - Wiring external systems into IdleHands
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your IdleHands config directory (typically `~/.idlehands/hooks/transforms`).
...
- `idlehands webhooks gmail setup` writes `hooks.gmail` config for `idlehands webhooks gmail run`.
