...
- Jobs persist under `~/.idlehands/cron/` so restarts don’t lose schedules.
...
idlehands cron list
idlehands cron run <job-id>
idlehands cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.idlehands/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `idlehands cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, IdleHands applies a
...
If `delivery` is omitted for isolated jobs, IdleHands defaults to `announce`.
...
- Job store: `~/.idlehands/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.idlehands/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
```json5
{
  cron: {
    ...
    store: "~/.idlehands/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, IdleHands archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `idlehands cron runs` and adjust retention before logs become large
...
idlehands cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
idlehands cron edit <jobId> --agent ops
idlehands cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
idlehands cron run <jobId>
idlehands cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
idlehands cron edit <jobId> \
...
```bash
idlehands cron edit <jobId> --exact
```
...
```bash
idlehands system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`idlehands system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `IDLEHANDS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- IdleHands applies exponential retry backoff for recurring jobs after consecutive errors:
