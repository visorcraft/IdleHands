...
read_when:
  - You want to place an outbound voice call from IdleHands
...
Voice calls for IdleHands via a plugin. Supports outbound notifications and
...
- Use `idlehands voicecall ...` or the `voice_call` tool
...
```bash
idlehands plugins install ./extensions/voice-call
