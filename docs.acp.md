# IdleHands ACP Bridge

This document describes how the IdleHands ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`idlehands acp` exposes an ACP agent over stdio and forwards prompts to a running
IdleHands Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `idlehands acp` and speaks ACP messages over stdio.
...
`idlehands acp` resolves the Gateway URL and auth from CLI flags or config:
