...
type EnvSnapshot = {
  ...
  idlehandsHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    idlehandsHome: process.env.IDLEHANDS_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("IDLEHANDS_HOME", snapshot.idlehandsHome);
  restoreKey("IDLEHANDS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked IDLEHANDS_HOME.
  delete process.env.IDLEHANDS_HOME;
  process.env.IDLEHANDS_STATE_DIR = path.join(base, ".idlehands");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "idlehands-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".idlehands", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
