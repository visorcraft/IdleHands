...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `idlehands-e2e-${name}-`));
  const configDir = path.join(homeDir, ".idlehands");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          IDLEHANDS_SKIP_PROVIDERS: "1",
          IDLEHANDS_SKIP_GMAIL_WATCHER: "1",
          IDLEHANDS_SKIP_CRON: "1",
          IDLEHANDS_SKIP_BROWSER_CONTROL_SERVER: "1",
          IDLEHANDS_SKIP_CANVAS_HOST: "1",
          IDLEHANDS_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
