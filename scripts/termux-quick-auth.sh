...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/idlehands/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "IdleHands Re-Auth" -c "After getting key, run: ssh $SERVER '~/idlehands/scripts/mobile-reauth.sh'" --id idlehands-auth
        ;;
    *)
