#!/bin/bash
# Setup IdleHands Auth Management System
...
echo "=== IdleHands Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'idlehands-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  IdleHands message: Send warning via IdleHands itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/idlehands-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/idlehands-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/idlehands-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now idlehands-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status idlehands-auth-monitor.timer"
echo ""
echo "Quick commands:"
