...
const shouldBuild = (deps) => {
  if (deps.env.IDLEHANDS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.IDLEHANDS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[idlehands] ${message}\n`);
};

const runIdleHands = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["idlehands.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runIdleHands(deps);
  }
