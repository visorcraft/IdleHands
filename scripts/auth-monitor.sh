...
#
# Suggested cron: */30 * * * * /home/admin/idlehands/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send IdleHands notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., idlehands-alerts)
...
STATE_FILE="$HOME/.idlehands/auth-monitor-state"
...
send_notification() {
    ...
    # Send via IdleHands if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use idlehands
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via IdleHands to $NOTIFY_PHONE..."
            idlehands send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! IdleHands is down. Run: ssh l36 '~/idlehands/scripts/mobile-reauth.sh'" "urgent"
    exit 1
