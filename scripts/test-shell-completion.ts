 ...
 * `idlehands update`. Use it to verify the completion installation flow
...
\`idlehands update\`, \`idlehands doctor\`, and \`idlehands onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# IdleHands Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`idlehands completion --install\` later to enable.`));
    return;
  }
