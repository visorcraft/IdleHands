...
IMAGE_NAME="idlehands-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="idlehands-net-e2e-$$"
GW_NAME="idlehands-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "IDLEHANDS_GATEWAY_TOKEN=$TOKEN" \
  -e "IDLEHANDS_SKIP_CHANNELS=1" \
  -e "IDLEHANDS_SKIP_GMAIL_WATCHER=1" \
  -e "IDLEHANDS_SKIP_CRON=1" \
  -e "IDLEHANDS_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
