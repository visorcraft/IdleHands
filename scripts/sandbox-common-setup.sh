...
BASE_IMAGE="${BASE_IMAGE:-idlehands-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-idlehands-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=idlehands.sandbox=1)
NOTE
