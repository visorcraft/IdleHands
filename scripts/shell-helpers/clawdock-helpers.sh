#!/usr/bin/env bash
# ClawDock - Docker helpers for IdleHands
# Inspired by Simon Willison's "Running IdleHands in Docker"
# https://til.simonwillison.net/llms/idlehands-docker
#
# Installation:
...
# Common paths to check for IdleHands
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/idlehands"
  "${HOME}/workspace/idlehands"
  "${HOME}/projects/idlehands"
  "${HOME}/dev/idlehands"
  "${HOME}/code/idlehands"
  "${HOME}/src/idlehands"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🖐️ Found IdleHands at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/idlehands"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ IdleHands not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/idlehands/idlehands.git ~/idlehands"
    echo "  cd ~/idlehands && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^IDLEHANDS_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d idlehands-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart idlehands-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f idlehands-gateway
}
...
clawdock-workspace() {
  cd ~/.idlehands/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec idlehands-gateway \
    bash -c 'echo "alias idlehands=\"./idlehands.mjs\"" > /tmp/.bashrc_idlehands && bash --rcfile /tmp/.bashrc_idlehands'
}
...
clawdock-cli() {
  _clawdock_compose run --rm idlehands-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build idlehands-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "IDLEHANDS_GATEWAY_TOKEN=$token" idlehands-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" idlehands-gateway \
    bash -c './idlehands.mjs config set gateway.remote.token "$TOKEN" && ./idlehands.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec idlehands-gateway \
    bash -c "./idlehands.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart idlehands-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm idlehands-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec idlehands-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'idlehands config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🖐️ ClawDock - Docker Helpers for IdleHands${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (idlehands alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to idlehands project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.idlehands)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'idlehands channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'idlehands status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.idlehands.ai${_CLR_RESET}"
  echo ""
}
