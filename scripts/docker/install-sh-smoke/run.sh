...
INSTALL_URL="${IDLEHANDS_INSTALL_URL:-https://idlehands.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${IDLEHANDS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${IDLEHANDS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="idlehands"
PACKAGE_NAME="${IDLEHANDS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (IDLEHANDS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${IDLEHANDS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${IDLEHANDS_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
