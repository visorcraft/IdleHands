...
# Build and bundle IdleHands into a minimal .app we can open.
# Outputs to dist/IdleHands.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/IdleHands.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="IdleHands"
BUNDLE_ID="${BUNDLE_ID:-ai.idlehands.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/idlehands/idlehands/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/IdleHands/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :IdleHandsBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :IdleHandsGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/IdleHands"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/IdleHands"
fi
chmod +x "$APP_ROOT/Contents/MacOS/IdleHands"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/IdleHands" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/IdleHands/Resources/IdleHands.icns" "$APP_ROOT/Contents/Resources/IdleHands.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/IdleHands/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying IdleHandsKit resources"
IDLEHANDSKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/IdleHandsKit_IdleHandsKit.bundle"
if [ -d "$IDLEHANDSKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/IdleHandsKit_IdleHandsKit.bundle"
  cp -R "$IDLEHANDSKIT_BUNDLE" "$APP_ROOT/Contents/Resources/IdleHandsKit_IdleHandsKit.bundle"
else
  echo "WARN: IdleHandsKit resource bundle not found at $IDLEHANDSKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running IdleHands"
killall -q IdleHands 2>/dev/null || true
