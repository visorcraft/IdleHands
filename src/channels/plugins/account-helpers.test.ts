...
function cfg(accounts?: Record<string, unknown> | null): IdleHandsConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as IdleHandsConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as IdleHandsConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as IdleHandsConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as IdleHandsConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as IdleHandsConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as IdleHandsConfig)).toBe("default");
    });
  });
});
