...
function telegramCfg(): IdleHandsConfig {
  return { channels: { telegram: { botToken: "tok" } } } as IdleHandsConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: IdleHandsConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as IdleHandsConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as IdleHandsConfig;
...
function createSignalAccountOverrideCfg(): IdleHandsConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as IdleHandsConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as IdleHandsConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as IdleHandsConfig,
        accountId: undefined,
        params: {
