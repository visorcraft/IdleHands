...
function resolveVoiceTtsConfig(params: { cfg: IdleHandsConfig; override?: TtsConfig }): {
  cfg: IdleHandsConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredIdleHandsTmpDir(), "discord-voice-"));
