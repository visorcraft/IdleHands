...
describe("agent components", () => {
  const createCfg = (): IdleHandsConfig => ({}) as IdleHandsConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): IdleHandsConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/idlehands-sessions-test.json");
  });
