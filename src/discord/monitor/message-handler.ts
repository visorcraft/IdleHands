      ...
      if (messageText) {
        ...
        if (isAbortRequestText(messageText)) {
          ...
          const route = resolveAgentRoute({
            cfg: params.cfg,
            channel: "discord",
            peer: { kind: "direct", id: channelId ?? String(data.channel_id) },
          });
          const sessionKey = route.sessionKey;
          const { sessionId, store } = resolveSessionForEmergencyStop(sessionKey);
          emergencyStop({ cfg: params.cfg, sessionKey, sessionId, sessionStore: store });
          // Ack via Discord reply if possible
          if (data.message && typeof (data.message as { reply?: unknown }).reply === "function") {
            await (data.message as { reply: (text: string) => Promise<void> }).reply(
              "⚙️ Agent was aborted.",
            ).catch(() => {});
          }
          return;
