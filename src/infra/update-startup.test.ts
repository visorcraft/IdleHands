...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["IDLEHANDS_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.IDLEHANDS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveIdleHandsPackageRoot } = await import("./idlehands-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveIdleHandsPackageRoot).mockResolvedValue("/opt/idlehands");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/idlehands/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
