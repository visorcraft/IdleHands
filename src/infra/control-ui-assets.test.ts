...
vi.mock("./idlehands-root.js", () => ({
  resolveIdleHandsPackageRoot: vi.fn(async () => null),
  resolveIdleHandsPackageRootSync: vi.fn(() => null),
}));
...
let idlehandsRoot: typeof import("./idlehands-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    idlehandsRoot = await import("./idlehands-root.js");
  });
  ...
  it("uses resolveIdleHandsPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/idlehands");
    (
      idlehandsRoot.resolveIdleHandsPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/idlehands"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "idlehands" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "idlehands.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-idlehands");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/idlehands-bundle");
    (
      idlehandsRoot.resolveIdleHandsPackageRootSync as unknown as ReturnType<typeof vi.fn>
