...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: IdleHandsConfig | null = null;
  ...
  get(key: string, cfg: IdleHandsConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: IdleHandsConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: IdleHandsConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: IdleHandsConfig): void {
