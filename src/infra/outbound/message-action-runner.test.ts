...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred IdleHands tmp root", async () => {
    const tmpRoot = resolvePreferredIdleHandsTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideIdleHands = path.join(os.tmpdir(), "outside-idlehands", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
