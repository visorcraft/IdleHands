...
import { POSIX_IDLEHANDS_TMP_DIR, resolvePreferredIdleHandsTmpDir } from "./tmp-idlehands-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredIdleHandsTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `idlehands-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_IDLEHANDS_TMP_DIR) {
  ...
  const resolved = resolvePreferredIdleHandsTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredIdleHandsTmpDir", () => {
  it("prefers /tmp/idlehands when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_IDLEHANDS_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/idlehands when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_IDLEHANDS_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/idlehands when /tmp/idlehands is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/idlehands when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/idlehands is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/idlehands is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/idlehands is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback IdleHands temp dir/);
  });
