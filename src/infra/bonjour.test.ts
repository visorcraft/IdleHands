...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.IDLEHANDS_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("idlehands-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("idlehands (IdleHands)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("idlehands");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("idlehands.local");
