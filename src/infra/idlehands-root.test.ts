...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__idlehands_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "idlehands-root");
...
describe("resolveIdleHandsPackageRoot", () => {
  let resolveIdleHandsPackageRoot: typeof import("./idlehands-root.js").resolveIdleHandsPackageRoot;
  let resolveIdleHandsPackageRootSync: typeof import("./idlehands-root.js").resolveIdleHandsPackageRootSync;

  beforeAll(async () => {
    ({ resolveIdleHandsPackageRoot, resolveIdleHandsPackageRootSync } =
      await import("./idlehands-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "idlehands");
    const pkgRoot = path.join(project, "node_modules", "idlehands");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "idlehands" }));

    expect(resolveIdleHandsPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "idlehands");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "idlehands.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "idlehands" }));

    expect(resolveIdleHandsPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveIdleHandsPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-idlehands package roots", async () => {
    const pkgRoot = fx("not-idlehands");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-idlehands" }));

    expect(resolveIdleHandsPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveIdleHandsPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveIdleHandsPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
