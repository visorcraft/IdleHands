...
function candidateBinDirs(opts: EnsureIdleHandsPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `idlehands` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "idlehands");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.IDLEHANDS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "idlehands"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `idlehands` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureIdleHandsCliOnPath(opts: EnsureIdleHandsPathOpts = {}) {
  if (isTruthyEnvValue(process.env.IDLEHANDS_PATH_BOOTSTRAPPED)) {
    return;
  }
