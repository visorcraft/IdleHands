...
export function triggerIdleHandsRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.IDLEHANDS_SYSTEMD_UNIT,
        process.env.IDLEHANDS_PROFILE,
      );
  ...
  const label =
    process.env.IDLEHANDS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.IDLEHANDS_PROFILE);
