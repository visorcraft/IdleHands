...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.IDLEHANDS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*idlehands-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "idlehands";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "idlehands");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "idlehands-gw");
  ...
  records.push(`_idlehands-gw._tcp IN PTR ${instanceLabel}._idlehands-gw._tcp`);
  records.push(`${instanceLabel}._idlehands-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._idlehands-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; idlehands-content-hash: ${contentHash}\n${contentBody}`;
}
