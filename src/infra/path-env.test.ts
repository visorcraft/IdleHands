...
let ensureIdleHandsCliOnPath: typeof import("./path-env.js").ensureIdleHandsCliOnPath;

describe("ensureIdleHandsCliOnPath", () => {
  const envKeys = [
    "PATH",
    "IDLEHANDS_PATH_BOOTSTRAPPED",
    "IDLEHANDS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureIdleHandsCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling idlehands exists", () => {
    const tmp = abs("/tmp/idlehands-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "idlehands");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.IDLEHANDS_PATH_BOOTSTRAPPED;

    ensureIdleHandsCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.IDLEHANDS_PATH_BOOTSTRAPPED = "1";
    ensureIdleHandsCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/idlehands-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "idlehands");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/idlehands-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "idlehands");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "idlehands");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/idlehands-path/case-linuxbrew");
