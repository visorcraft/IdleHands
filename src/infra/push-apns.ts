...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.IDLEHANDS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.IDLEHANDS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set IDLEHANDS_APNS_TEAM_ID and IDLEHANDS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.IDLEHANDS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.IDLEHANDS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.IDLEHANDS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set IDLEHANDS_APNS_PRIVATE_KEY_P8 or IDLEHANDS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading IDLEHANDS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createIdleHandsPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    idlehands: createIdleHandsPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
