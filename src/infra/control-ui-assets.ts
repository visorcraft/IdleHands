...
import { resolveIdleHandsPackageRoot, resolveIdleHandsPackageRootSync } from "./idlehands-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveIdleHandsPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "idlehands" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveIdleHandsPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // idlehands.mjs or dist/<bundle>.js
