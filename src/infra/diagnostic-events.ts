...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __idlehandsDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__idlehandsDiagnosticEventsState) {
    globalStore.__idlehandsDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__idlehandsDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: IdleHandsConfig): boolean {
