...
declare const __IDLEHANDS_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "idlehands";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["IDLEHANDS_VERSION"],
      env["IDLEHANDS_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current IdleHands version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __IDLEHANDS_VERSION__ === "string" && __IDLEHANDS_VERSION__) ||
  process.env.IDLEHANDS_BUNDLED_VERSION ||
