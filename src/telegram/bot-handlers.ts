...
export const registerTelegramHandlers = ({
  ...
  const processInboundMessage = async (params: {
    ...
    // /stop bypasses ALL queues, debouncing, text-fragment buffering, and
    // media-group handling. It executes immediately at the earliest point.
    if (text) {
      const { isAbortRequestText, emergencyStop, resolveSessionForEmergencyStop } = await import(
        "../auto-reply/reply/emergency-stop.js"
      );
      if (isAbortRequestText(text)) {
        const senderId = msg.from?.id ? String(msg.from.id) : "";
        const isAuthorized =
          !senderId || !allowFrom?.length || allowFrom.includes(senderId);
        if (isAuthorized) {
          ...
          const sessionKey = route.sessionKey;
          const { sessionId, store } = resolveSessionForEmergencyStop(sessionKey);
          ...
          logVerbose(`telegram: emergency-stop fast-path fired for ${sessionKey}`);
          // Send ack immediately — do not await the debouncer or processMessage
          await withTelegramApiErrorLogging({
            operation: "sendMessage",
            runtime,
            fn: () =>
              bot.api.sendMessage(chatId, result.replyText, {
