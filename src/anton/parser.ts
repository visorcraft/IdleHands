...
export function findRunnablePendingTasks(
  ...
  // Parents with pending children delegate to children directly.
  const delegatingParentKeys = new Set(
    pendingTasks
      .filter((t) => t.children.length > 0 && t.children.some((c) => !c.checked))
      .map((t) => t.key)
  );
  ...
  return pendingTasks.filter((task) => {
    if (delegatingParentKeys.has(task.key)) return false;

    if (!task.parentKey) return true;

    const parent = taskFile.allTasks.find((t) => t.key === task.parentKey);
    if (!parent) return true;
    if (parent.checked) return true;

    // Children are blocked by pending parents, UNLESS that parent is delegating
    if (pendingKeys.has(parent.key) && !delegatingParentKeys.has(parent.key)) {
      return false;
    }
...
export async function insertSubTasks(
  ...
  const newTasks = updatedTaskFile.allTasks.filter(
    (task) =>
      task.parentKey === parentKey && !taskFile.allTasks.some((oldTask) => oldTask.key === task.key)
  );
