...
import { renderTelegramHtml } from '../bot/ux/telegram-renderer.js';
import {
    ...
    if (this.messageId) {
      this.scheduler = new MessageEditScheduler({
        ...
        render: () => {
          ...
          const blocks = snapshotToBlocks(snap, this.presenter['banner']);
          return renderTelegramHtml(blocks, { maxLen: this.presenter.getBudgets().telegramMaxLen });
        },
...
export async function startTelegramBot(
  ...
  if (watchdogSettings.clamped) {
    console.error(
      `[bot:telegram] watchdog_timeout_ms was clamped to 30000ms (configured value was below minimum)`
    );
  }
    ...
    while (!askComplete) {
      ...
      try {
        ...
        if (isAbort && watchdogCompactPending) {
          ...
          const compactWaitStart = Date.now();
          const compactWaitMax = 60_000; // 60s max wait for compaction
          while (watchdogCompactPending) {
            if (Date.now() - compactWaitStart > compactWaitMax) {
              console.error(
                `[bot:telegram] ${managed.chatId} compaction wait timed out after ${compactWaitMax}ms — giving up`
              );
              watchdogCompactPending = false;
              watchdogForcedCancel = true;
              break;
          ...
          if (watchdogForcedCancel) {
            // Compaction timed out, don't retry
            askComplete = true;
            if (sessions.isTurnActive(managed.chatId, turnId)) {
              await streaming.finalizeError(
                'Cancelled — context compaction timed out. Try a smaller scope or faster model.'
              );
            }
            break;
          }
        ...
        if (
          ...
          console.error(
            `[bot:telegram] ${managed.chatId} tool-loop auto-continue (retry ${toolLoopRetryCount}/${autoContinueMaxRetries})`
          );
          if (sessions.isTurnActive(managed.chatId, turnId)) {
