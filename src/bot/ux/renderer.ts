...
import type { TurnProgressSnapshot } from '../../progress/turn-progress.js';
...
// ---------------------------------------------------------------------------
// Snapshot to Blocks Conversion (for ProgressPresenter)
// ---------------------------------------------------------------------------

/**
 * Convert ProgressPresenter snapshot to UX blocks for shared rendering.
 * This function is used by both Telegram and Discord handlers to avoid
 * duplicating message composition logic.
 *
 * @param snap - The TurnProgressSnapshot from ProgressPresenter
 * @param banner - Optional banner text to display at the top
 * @returns Array of UX blocks ready for platform-specific rendering
 */
export function snapshotToBlocks(snap: TurnProgressSnapshot, banner?: string | null): UXBlock[] {
  const blocks: UXBlock[] = [];

  // Header banner (if any)
  if (banner) {
    blocks.push({
      type: 'message',
      category: 'PROGRESS',
  ...
  // Tool lines
  if (snap.toolLines && snap.toolLines.length > 0) {
    blocks.push({
      type: 'section',
      title: 'Tools',
      content: snap.toolLines.map((line: string) => ({
        type: 'text',
        content: line,
      })),
    });
