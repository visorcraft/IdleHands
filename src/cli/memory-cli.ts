    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["idlehands memory status", "Show index and provider status."],
          ["idlehands memory index --force", "Force a full reindex."],
          ['idlehands memory search --query "deployment notes"', "Search indexed memory entries."],
          ["idlehands memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.idlehands.ai/cli/memory")}\n`,
    );
