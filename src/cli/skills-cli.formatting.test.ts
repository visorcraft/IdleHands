...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "idlehands-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "idlehands-bundled-skills-test-"));
    process.env.IDLEHANDS_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
