...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    IDLEHANDS_STATE_DIR: "/tmp/idlehands-daemon",
    IDLEHANDS_CONFIG_PATH: "/tmp/idlehands-daemon/idlehands.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.IDLEHANDS_STATE_DIR ?? "/tmp/idlehands-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.IDLEHANDS_CONFIG_PATH ?? `${stateDir}/idlehands.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/idlehands-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.IDLEHANDS_STATE_DIR = "/tmp/idlehands-cli";
    process.env.IDLEHANDS_CONFIG_PATH = "/tmp/idlehands-cli/idlehands.json";
