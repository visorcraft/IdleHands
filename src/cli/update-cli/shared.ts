...
import { resolveIdleHandsPackageRoot } from "../../infra/idlehands-root.js";
...
const IDLEHANDS_REPO_URL = "https://github.com/idlehands/idlehands.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "idlehands";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("idlehands@")) {
    return trimmed.slice("idlehands@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.IDLEHANDS_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveIdleHandsPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", IDLEHANDS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `IDLEHANDS_GIT_DIR points at a non-git directory: ${params.dir}. Set IDLEHANDS_GIT_DIR to an empty folder or an idlehands checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`IDLEHANDS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "idlehands.mjs");
