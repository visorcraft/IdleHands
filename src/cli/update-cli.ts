...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["idlehands update", "Update a source checkout (git)"],
        ["idlehands update --channel beta", "Switch to beta channel (git + npm)"],
        ["idlehands update --channel dev", "Switch to dev channel (git + npm)"],
        ["idlehands update --tag beta", "One-off update to a dist-tag or version"],
        ["idlehands update --dry-run", "Preview actions without changing anything"],
        ["idlehands update --no-restart", "Update without restarting the service"],
        ["idlehands update --json", "Output result as JSON"],
        ["idlehands update --yes", "Non-interactive (accept downgrade prompts)"],
        ["idlehands update wizard", "Interactive update wizard"],
        ["idlehands --update", "Shorthand for idlehands update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run idlehands update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.idlehands.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["idlehands update status", "Show channel + version status."],
          ["idlehands update status --json", "JSON output."],
          ["idlehands update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.idlehands.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
