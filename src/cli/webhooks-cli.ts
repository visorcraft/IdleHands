...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.idlehands.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + IdleHands hooks")
    ...
    .option("--hook-url <url>", "IdleHands hook URL")
    .option("--hook-token <token>", "IdleHands hook token")
