...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("idlehands node install"), formatCliCommand("idlehands node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("idlehands node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    IDLEHANDS_LOG_PREFIX: baseEnv.IDLEHANDS_LOG_PREFIX ?? "node",
