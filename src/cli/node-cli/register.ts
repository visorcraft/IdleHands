    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "idlehands node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["idlehands node status", "Check node host service status."],
          ["idlehands node install", "Install the node host service."],
          ["idlehands node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.idlehands.ai/cli/node")}\n`,
    );
