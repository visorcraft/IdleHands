...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@idlehands/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@idlehands/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@idlehands/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@idlehands/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @idlehands/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@idlehands/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@idlehands/plugin-alpha@1.2.3",
        installPath: "/tmp/idlehands/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@idlehands/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @idlehands/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@idlehands/plugin-alpha@latest",
      true,
      "/tmp/idlehands/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@idlehands/plugin-alpha@latest",
      installPath: "/tmp/idlehands/extensions/alpha",
      version: "1.2.3",
