...
const entries: SubCliEntry[] = [
  ...
  {
    name: "upgrade",
    description: "Upgrade IdleHands to the latest version (simple: check → install → restart)",
    hasSubcommands: false,
    register: async (program) => {
      program
        .command("upgrade")
        .description("Upgrade IdleHands to the latest version (check → install → restart)")
        .option("--check", "Only check for updates without installing", false)
        .action(async (opts) => {
          const { performBotUpgrade, checkBotUpdate } =
            await import("../../bot/upgrade-command.js");

          if (opts.check) {
            const info = await checkBotUpdate();
            if (info.available) {
              console.log(`Update available: v${info.current} → v${info.latest} (${info.source})`);
            } else {
              console.log(`Already on the latest version (v${info.current}).`);
            }
            return;
          }
