...
import type { IdleHandsConfig, ConfigFileSnapshot } from "../config/types.idlehands.js";
...
vi.mock("../infra/idlehands-root.js", () => ({
  resolveIdleHandsPackageRoot: vi.fn(),
}));
...
const { resolveIdleHandsPackageRoot } = await import("../infra/idlehands-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/idlehands-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as IdleHandsConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/idlehands-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveIdleHandsPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("idlehands-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveIdleHandsPackageRoot).mockClear();
    ...
    vi.mocked(resolveIdleHandsPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("idlehands");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/idlehands-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "idlehands-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("IdleHands update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as IdleHandsConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveIdleHandsPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("idlehands-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ IDLEHANDS_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.IDLEHANDS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("idlehands-update-wizard");
    await withEnvAsync({ IDLEHANDS_GIT_DIR: tempDir }, async () => {
      setTty(true);
