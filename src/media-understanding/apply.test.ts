...
const TEMP_MEDIA_PREFIX = "idlehands-media-";
...
function createGroqAudioConfig(): IdleHandsConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): IdleHandsConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): IdleHandsConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: IdleHandsConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredIdleHandsTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        IDLEHANDS_AGENT_DIR: isolatedAgentDir,
