...
import { BenchCase, BenchResult } from './types.js';
...
async function main() {
  ...
  const iterTimeoutSec =
    Number.isFinite(Number(process.env.IDLEHANDS_BENCH_ITER_TIMEOUT_SEC))
  ...
  for (let i = 0; i < reps; i++) {
    ...
    try {
      console.error(`[bench] case=${c.name} engine=idlehands iter=${i + 1}/${reps} reuse_session=${reuse}`);
      if (reuse && i > 0) {
        session.reset();
      }
      if (!reuse) {
      ...
      const res = await session
        ...
        .catch(async (e: any) => {
          ...
          const retryable = /fetch failed|Connection timeout|Cannot reach|model loading|returned 503/i.test(message);
          if (retryable) {
            try {
              const { OpenAIClient } = await import('../client.js');
              const cl = new OpenAIClient(config.endpoint, config.api_key, false);
              console.error(`[bench] transient server error (${message}); waiting for /v1/models ...`);
      ...
      if (c.success) {
        if (c.success.type === 'equals') {
          ...
          if (!ok) reason = `expected ${JSON.stringify(c.success.value)}, got ${JSON.stringify(out)}`;
      ...
      console.error(`[bench] result case=${c.name} engine=idlehands iter=${row.iter}/${reps} ok=${row.ok} ttc_s=${ttcS} ttfr_s=${ttfrS} reason=${row.ok ? 'ok' : row.reason}`);
      ...
      if (retryToSuccess) {
        const retryable = /fetch failed|Connection timeout|Cannot reach|model loading|returned 503/i.test(reason);
        if (retryable) {
          const tries = (retryCounts.get(i) ?? 0) + 1;
          retryCounts.set(i, tries);
          if (tries <= maxRetry) {
            console.error(`[bench] retrying iter ${i + 1}/${reps} after retryable error (${tries}/${maxRetry}): ${reason}`);
            i -= 1;
            continue;
          }
        }
      ...
      console.error(`[bench] result case=${c.name} engine=idlehands iter=${row.iter}/${reps} ok=false reason=${row.reason}`);
      results.push(row);
    } finally {
  ...
  if (!process.env.IDLEHANDS_GIT_SHA) {
    try {
      ...
      process.env.IDLEHANDS_GIT_SHA = execSync('git rev-parse --short HEAD', { encoding: 'utf8' }).trim();
    } catch {}
  }
