 ...
 * A common mistake — caused by the similarity with the `idlehands.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (idlehands.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.IDLEHANDS_AUTH_STORE_READONLY === "1";
