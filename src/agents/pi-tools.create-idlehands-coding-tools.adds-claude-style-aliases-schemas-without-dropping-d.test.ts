...
describe("createIdleHandsCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-read-"));
    try {
      ...
      const contents = "Hello from idlehands read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createIdleHandsCodingTools({ sandbox });
