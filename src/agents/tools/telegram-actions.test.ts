...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): IdleHandsConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as IdleHandsConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): IdleHandsConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as IdleHandsConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: IdleHandsConfig, accountId = "media") {
    await handleTelegramAction(
