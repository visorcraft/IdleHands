...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    idlehands: process.env.IDLEHANDS_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.idlehands === undefined) {
      delete process.env.IDLEHANDS_GATEWAY_TOKEN;
    } else {
      process.env.IDLEHANDS_GATEWAY_TOKEN = envSnapshot.idlehands;
    }
  ...
  it("uses IDLEHANDS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.IDLEHANDS_GATEWAY_TOKEN = "local-env-token";
