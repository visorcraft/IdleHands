...
type WebSearchConfig = NonNullable<IdleHandsConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: IdleHandsConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("idlehands configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.idlehands.ai/tools/web",
  };
}
