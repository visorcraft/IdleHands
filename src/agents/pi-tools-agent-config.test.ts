...
import { createIdleHandsCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: IdleHandsConfig) {
    return createIdleHandsCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<IdleHandsConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<IdleHandsConfig["agents"]>["list"]>[number]["tools"];
  }): IdleHandsConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createIdleHandsCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createIdleHandsCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createIdleHandsCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createIdleHandsCodingTools({
      config: cfg,
    ...
    const bobTools = createIdleHandsCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createIdleHandsCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createIdleHandsCodingTools({
      config: cfg,
