...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const idleHandsTmpDir = path.resolve(resolvePreferredIdleHandsTmpDir());
  if (!isPathInside(idleHandsTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: idleHandsTmpDir });
  return resolved;
}
