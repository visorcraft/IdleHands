...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("idlehands-ws-", async (workspaceDir) => {
      await withTempDir("idlehands-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createIdleHandsCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("idlehands-ws-", async (workspaceDir) => {
      await withTempDir("idlehands-override-", async (overrideDir) => {
        const tools = createIdleHandsCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("idlehands-ws-", async (workspaceDir) => {
      const cfg: IdleHandsConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createIdleHandsCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-idlehands.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("idlehands-sandbox-", async (sandboxDir) => {
      await withTempDir("idlehands-workspace-", async (workspaceDir) => {
        ...
        const tools = createIdleHandsCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
