...
async function withOutsideHardlinkInIdleHandsTmp<T>(
  ...
  const hardlinkPath = path.join(params.idleHandsTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.idleHandsTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.idleHandsTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.idleHandsTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const idleHandsTmpDir = resolvePreferredIdleHandsTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred IdleHands tmp root",
      media: path.join(idleHandsTmpDir, "image.png"),
      expected: path.join(idleHandsTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred IdleHands tmp root",
      media: pathToFileURL(path.join(idleHandsTmpDir, "photo.png")).href,
      expected: path.join(idleHandsTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred IdleHands tmp root",
      media: path.join(idleHandsTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(idleHandsTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(idleHandsTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked IdleHands tmp paths escaping tmp root", async () => {
    ...
    if (isPathInside(idleHandsTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(idleHandsTmpDir, { recursive: true });
      const symlinkPath = path.join(idleHandsTmpDir, `tmp-link-escape-${process.pid}`);
