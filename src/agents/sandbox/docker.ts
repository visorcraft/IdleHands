...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "idlehands.sandbox=1");
  args.push("--label", `idlehands.sessionKey=${params.scopeKey}`);
  args.push("--label", `idlehands.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `idlehands.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "idlehands.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`idlehands sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`idlehands sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("idlehands sandbox recreate --all");
}
