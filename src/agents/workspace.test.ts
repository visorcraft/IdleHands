...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses IDLEHANDS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      IDLEHANDS_HOME: "/srv/idlehands-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/idlehands-home"), ".idlehands", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".idlehands", "workspace-state.json"] as const;
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-workspace-hardlink-"));
    try {
