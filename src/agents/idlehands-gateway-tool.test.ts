...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createIdleHandsTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-test-"));

    try {
      await withEnvAsync(
        { IDLEHANDS_STATE_DIR: stateDir, IDLEHANDS_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: idlehands --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/idlehands" } }\n}\n';
