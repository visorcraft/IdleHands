...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["IDLEHANDS_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-subagent-"));
  ...
  it("uses isolated temp state when IDLEHANDS_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "idlehands-test-state"));
  });
});
