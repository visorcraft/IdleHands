...
const GOOGLE_LIVE_SINGLE_KEY = "IDLEHANDS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "IDLEHANDS_LIVE_ANTHROPIC_KEY",
    listVar: "IDLEHANDS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "IDLEHANDS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `IDLEHANDS_LIVE_${base}_KEY`;
