...
const defaultTools = createIdleHandsCodingTools({ senderIsOwner: true });
...
describe("createIdleHandsCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: IdleHandsConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createIdleHandsCodingTools({
    ...
    const anthropicTools = createIdleHandsCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: IdleHandsConfig = {
      tools: {
        exec: {
    ...
    const allowed = createIdleHandsCodingTools({
    ...
    const denied = createIdleHandsCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createIdleHandsCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
