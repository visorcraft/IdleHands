...
describe("resolveIdleHandsAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          IDLEHANDS_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveIdleHandsAgentDir();
  ...
  it("honors IDLEHANDS_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          IDLEHANDS_STATE_DIR: undefined,
          IDLEHANDS_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveIdleHandsAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when IDLEHANDS_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers IDLEHANDS_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          IDLEHANDS_STATE_DIR: undefined,
          IDLEHANDS_AGENT_DIR: primaryOverride,
