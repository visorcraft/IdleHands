...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("idlehands-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("idlehands-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("idlehands-sbx-");
