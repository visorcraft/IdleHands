...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: IdleHandsConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/idlehands" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: IdleHandsConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: IdleHandsConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: IdleHandsConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: IdleHandsConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: IdleHandsConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: IdleHandsConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: IdleHandsConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: IdleHandsConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/idlehands-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/idlehands");
  });

  it("uses IDLEHANDS_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "idlehands-home");
    vi.stubEnv("IDLEHANDS_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as IdleHandsConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".idlehands", "workspace"));
  });

  it("uses IDLEHANDS_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "idlehands-home");
    vi.stubEnv("IDLEHANDS_HOME", home);
    // Clear state dir so it falls back to IDLEHANDS_HOME
    vi.stubEnv("IDLEHANDS_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as IdleHandsConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".idlehands", "agents", "main", "agent"));
  });
});
