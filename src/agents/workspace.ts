...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.IDLEHANDS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".idlehands", `workspace-${profile}`);
  }
  return path.join(home, ".idlehands", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".idlehands";
...
export async function ensureAgentWorkspace(params?: {
  ...
  const identityEnabled = params?.agentIdentityEnabled ?? true;
