...
import type { ConfirmationProvider, ConfirmRequest, ApprovalMode, BlockedNotice } from '../types.js';

const READ_ONLY_TOOLS = new Set(['read_file', 'read_files', 'list_dir', 'search_files', 'sys_context',
  'vault_search', 'lsp_diagnostics', 'lsp_symbols', 'lsp_hover', 'lsp_definition', 'lsp_references']);
  ...
  async confirm(opts: ConfirmRequest): Promise<boolean> {
    ...
    switch (this.mode) {
      ...
      case 'reject':
        ...
        console.error(`[non-interactive] rejected ${opts.tool}: ${opts.summary ?? '(no summary)'} — use --no-confirm to auto-approve`);
        return false;
      case 'default':
      case 'plan':
        // Non-interactive — can't prompt, so reject
        console.error(`[headless] rejected ${opts.tool}: ${opts.summary} (mode=${this.mode}, no TTY)`);
        return false;
    }
  }
