...
export class VaultStore {
  ...
  async upsertNote(key: string, value: string, kind: 'note' | 'system' = 'note'): Promise<string> {
    ...
    const id = this.transaction(() => {
      ...
      if (existing.length) {
        ...
        currentId = this.insertAndIndex(kind, cleanKey, cleanVal, null, null, cleanVal, snippet);
      }
  ...
  async archiveToolMessages(
    ...
    const existingIds = new Set(
      this.rows<{ tool_call_id: string }>(
        `SELECT tool_call_id FROM vault_entries WHERE kind='tool' AND tool_call_id IN (${placeholders})`,
        toolCallIds
    ...
    const toArchive = messages
      .filter(isToolMessage)
      .filter((m) => !existingIds.has(m.tool_call_id) && m.content?.trim())
      .map((m) => ({
        content: m.content,
  ...
  private async rebuildFts() {
    ...
    try {
      const exists = this.rows<{ name: string }>(
