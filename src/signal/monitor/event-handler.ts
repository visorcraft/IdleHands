    ...
    if (bodyText) {
      ...
      if (isAbortRequestText(bodyText)) {
        ...
        const route = resolveAgentRoute({
          cfg: deps.cfg,
          channel: "signal",
          peer: { kind: isGroup ? "group" : "direct", id: isGroup ? (groupId ?? "") : senderPeerId },
        });
        const { sessionId, store } = resolveSessionForEmergencyStop(route.sessionKey);
        emergencyStop({ cfg: deps.cfg, sessionKey: route.sessionKey, sessionId, sessionStore: store });
        logVerbose(`signal: emergency-stop fast-path fired for ${route.sessionKey}`);
        return;
      }
    }
