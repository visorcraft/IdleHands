...
function isWhatsAppConfigured(cfg: IdleHandsConfig): boolean {
...
function isGenericChannelConfigured(cfg: IdleHandsConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: IdleHandsConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: IdleHandsConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: IdleHandsConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: IdleHandsConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: IdleHandsConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: IdleHandsConfig, pluginId: string): IdleHandsConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
