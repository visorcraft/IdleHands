...
import type { ConfigFileSnapshot } from "./types.idlehands.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.idlehands/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/idlehands/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/idlehands/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/idlehands/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/idlehands/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(IdleHandsSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = IdleHandsSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "IdleHandsConfig";
    const hints = mapSensitivePaths(IdleHandsSchema, "", {});
