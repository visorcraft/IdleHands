...
import type { IdleHandsConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareIdleHandsVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  idlehands config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  idlehands config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: IdleHandsConfig,
  pathSegments: string[],
): { changed: boolean; next: IdleHandsConfig } {
...
function coerceConfig(value: unknown): IdleHandsConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as IdleHandsConfig;
}
...
function stampConfigVersion(cfg: IdleHandsConfig): IdleHandsConfig {
...
function warnIfConfigFromFuture(cfg: IdleHandsConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareIdleHandsVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer IdleHands (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as IdleHandsConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): IdleHandsConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as IdleHandsConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: IdleHandsConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as IdleHandsConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.IDLEHANDS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.IDLEHANDS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.IDLEHANDS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.IDLEHANDS_WATCH_SESSION === "string" &&
        deps.env.IDLEHANDS_WATCH_SESSION.trim().length > 0
          ? deps.env.IDLEHANDS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.IDLEHANDS_WATCH_COMMAND === "string" &&
        deps.env.IDLEHANDS_WATCH_COMMAND.trim().length > 0
          ? deps.env.IDLEHANDS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `IDLEHANDS_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: IdleHandsConfig | null = null;
let runtimeConfigSourceSnapshot: IdleHandsConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.IDLEHANDS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.IDLEHANDS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): IdleHandsConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): IdleHandsConfig {
