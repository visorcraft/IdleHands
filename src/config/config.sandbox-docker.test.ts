...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("idlehands-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "idlehands-sandbox-browser-global" },
      agentBrowser: { network: "idlehands-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("idlehands-sandbox-browser-agent");
  });
