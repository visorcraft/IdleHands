...
import type { IdleHandsConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: IdleHandsConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: IdleHandsConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = IdleHandsSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as IdleHandsConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as IdleHandsConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as IdleHandsConfig,
  };
}
