...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = IdleHandsSchema.safeParse({
      $schema: "https://idlehands.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://idlehands.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = IdleHandsSchema.safeParse({ $schema: 123 });
...
describe("ui.seamColor", () => {
  ...
  it("rejects non-hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "hand" } });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = IdleHandsSchema.safeParse({
      cron: {
        enabled: true,
