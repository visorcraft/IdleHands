...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "idlehands.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "idlehands.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.IDLEHANDS_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.IDLEHANDS_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.IDLEHANDS_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[idlehands] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[idlehands] ${parsed.error}`);
      process.exit(2);
    }
