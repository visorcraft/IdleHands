...
import { createIdleHandsTools } from "../agents/idlehands-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-idlehands-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-idlehands-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-idlehands-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-idlehands-thread-id")?.trim() || undefined;
  ...
  const allTools = createIdleHandsTools({
