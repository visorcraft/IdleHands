...
type GetReplyFromConfigFn = (
  ...
  configOverride?: IdleHandsConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `idlehands-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.IDLEHANDS_CONFIG_PATH = path.join(root, "idlehands.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "idlehands.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "idlehands-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadIdleHandsPlugins: () => pluginRegistryState.registry,
  };
});
