...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "IDLEHANDS_SKIP_BROWSER_CONTROL_SERVER",
  "IDLEHANDS_SKIP_GMAIL_WATCHER",
  "IDLEHANDS_SKIP_CANVAS_HOST",
  "IDLEHANDS_BUNDLED_PLUGINS_DIR",
  "IDLEHANDS_SKIP_CHANNELS",
  "IDLEHANDS_SKIP_PROVIDERS",
  "IDLEHANDS_SKIP_CRON",
  "IDLEHANDS_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.IDLEHANDS_STATE_DIR = path.join(tempHome, ".idlehands");
  delete process.env.IDLEHANDS_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.IDLEHANDS_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.IDLEHANDS_SKIP_GMAIL_WATCHER = "1";
  process.env.IDLEHANDS_SKIP_CANVAS_HOST = "1";
  process.env.IDLEHANDS_SKIP_CHANNELS = "1";
  process.env.IDLEHANDS_SKIP_PROVIDERS = "1";
  process.env.IDLEHANDS_SKIP_CRON = "1";
  process.env.IDLEHANDS_TEST_MINIMAL_GATEWAY = "1";
  process.env.IDLEHANDS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "idlehands-test-no-bundled-extensions")
    : "idlehands-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "idlehands-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".idlehands-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__idlehandsTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__idlehandsTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["IDLEHANDS_GATEWAY_TOKEN"]);
  const prev = process.env.IDLEHANDS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.IDLEHANDS_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.IDLEHANDS_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
