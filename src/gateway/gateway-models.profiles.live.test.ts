...
import type { ModelsConfig, IdleHandsConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.IDLEHANDS_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.IDLEHANDS_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.IDLEHANDS_LIVE_GATEWAY_PROVIDERS);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.IDLEHANDS_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.IDLEHANDS_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  cfg: IdleHandsConfig;
  agentDir: string;
}): IdleHandsConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<IdleHandsConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.IDLEHANDS_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.IDLEHANDS_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveIdleHandsAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-live-state-"));
  process.env.IDLEHANDS_STATE_DIR = tempStateDir;
  ...
  process.env.IDLEHANDS_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.idlehands-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveIdleHandsAgentDir();
  const sanitizedCfg: IdleHandsConfig = {
    ...params.cfg,
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "IdleHands live tool probe (local, safe): " +
    ...
    process.env.IDLEHANDS_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.IDLEHANDS_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.IDLEHANDS_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via IDLEHANDS_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.idlehands-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
