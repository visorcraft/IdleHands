...
export function resolveHooksConfig(cfg: IdleHandsConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: IdleHandsConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-idlehands-token"] === "string"
      ? req.headers["x-idlehands-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
