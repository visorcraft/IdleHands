...
let lastCreateIdleHandsToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/idlehands-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateIdleHandsToolsContext?.agentTo,
          agentThreadId: lastCreateIdleHandsToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createIdleHandsTools: (ctx: Record<string, unknown>) => {
      lastCreateIdleHandsToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateIdleHandsToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-idlehands-message-to": "channel:24514",
        "x-idlehands-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
