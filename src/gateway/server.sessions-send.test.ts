...
const { createIdleHandsTools } = await import("../agents/idlehands-tools.js");
...
type SessionSendTool = ReturnType<typeof createIdleHandsTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createIdleHandsTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["IDLEHANDS_GATEWAY_PORT", "IDLEHANDS_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.IDLEHANDS_GATEWAY_PORT = String(gatewayPort);
  process.env.IDLEHANDS_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("IDLEHANDS_CONFIG_PATH missing in gateway test environment");
      }
