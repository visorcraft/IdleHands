...
const connectNodeClient = async (params: {
  ...
  const token = process.env.IDLEHANDS_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("IDLEHANDS_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".idlehands", "restart-sentinel.json");
...
describe("gateway node command allowlist", () => {
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    ...
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `idlehands-spoof-test-device-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
