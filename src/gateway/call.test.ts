...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("idlehands doctor --fix");
  });
...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["IDLEHANDS_GATEWAY_TOKEN", "IDLEHANDS_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.IDLEHANDS_GATEWAY_TOKEN = "env-token";
    process.env.IDLEHANDS_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "IDLEHANDS_GATEWAY_PASSWORD",
      envValue: "from-env",
    ...
    },
    {
      label: "token",
      authKey: "token",
      envKey: "IDLEHANDS_GATEWAY_TOKEN",
      envValue: "env-token",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["IDLEHANDS_GATEWAY_PASSWORD", "IDLEHANDS_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.IDLEHANDS_GATEWAY_PASSWORD = envPassword;
    }
