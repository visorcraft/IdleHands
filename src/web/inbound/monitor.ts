      ...
      if (stopText) {
        ...
        if (isAbortRequestText(stopText)) {
          ...
          const route = resolveAgentRoute({
            cfg: freshCfg,
            channel: "whatsapp",
            peer: { kind: "direct", id: inboundMessage.from ?? "" },
          });
          const { sessionId, store } = resolveSessionForEmergencyStop(route.sessionKey);
          emergencyStop({ cfg: freshCfg, sessionKey: route.sessionKey, sessionId, sessionStore: store });
          if (typeof reply === "function") {
            await reply("⚙️ Agent was aborted.").catch(() => {});
          }
          return;
        }
      }
