...
describe("matchesMentionWithExplicit", () => {
  const mentionRegexes = [/\bidlehands\b/i];
  ...
  it("resolves known variables, aliases, and case-insensitive tokens", () => {
    const cases = [
      ...
      },
      {
        ...
        values: {
          identityName: "IdleHands",
          provider: "anthropic",
          model: "claude-opus-4-5",
          thinkingLevel: "high",
        },
        expected: "[IdleHands] anthropic/claude-opus-4-5 (think:high)",
      },
    ] as const;
    for (const testCase of cases) {
