...
export function resolveConfiguredAcpBackendId(cfg: IdleHandsConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: IdleHandsConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `idlehands plugins install ${localPath}`;
    }
    return "idlehands plugins install @idlehands/acpx";
  }
