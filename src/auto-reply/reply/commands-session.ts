...
import { scheduleGatewaySigusr1Restart, triggerIdleHandsRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting IdleHands in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerIdleHandsRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting IdleHands via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
