...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop idlehands!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@idlehands_bot", { botUsername: "idlehands_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@idlehands_bot", { botUsername: "idlehands_bot" })).toBe(true);
