...
function createReplyConfig(home: string, streamMode?: "block"): IdleHandsConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("idlehands-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("IDLEHANDS_TEST_FAST", "1");
