...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses IDLEHANDS_HOME for tilde expansion", () => {
    vi.stubEnv("IDLEHANDS_HOME", "/srv/idlehands-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/idlehands-home", "cron", "jobs.json"));
  });
});
