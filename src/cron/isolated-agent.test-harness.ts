...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "idlehands-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".idlehands", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<IdleHandsConfig> = {},
): IdleHandsConfig {
  const base: IdleHandsConfig = {
    agents: {
      defaults: {
