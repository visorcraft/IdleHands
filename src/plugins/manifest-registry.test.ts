...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `idlehands-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "idlehands.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("rejects manifest paths that escape plugin root via symlink", () => {
    ...
    const outsideManifest = path.join(outsideDir, "idlehands.plugin.json");
    const linkedManifest = path.join(rootDir, "idlehands.plugin.json");
