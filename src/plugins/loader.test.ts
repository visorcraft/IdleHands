...
import { __testing, loadIdleHandsPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `idlehands-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.IDLEHANDS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "idlehands.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.IDLEHANDS_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadIdleHandsPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadIdleHandsPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.IDLEHANDS_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.IDLEHANDS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadIdleHandsPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadIdleHandsPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadIdleHandsPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.IDLEHANDS_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ IDLEHANDS_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
