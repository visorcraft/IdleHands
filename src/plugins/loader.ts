...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: IdleHandsPluginDefinition;
  register?: IdleHandsPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as IdleHandsPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as IdleHandsPluginDefinition;
...
export function loadIdleHandsPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverIdleHandsPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "idlehands/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "idlehands/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: IdleHandsPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as IdleHandsPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
