    ...
    if (msgText) {
      ...
      if (isAbortRequestText(msgText)) {
        ...
        const route = resolveAgentRoute({
          cfg,
          channel: "imessage",
          peer: { kind: "direct", id: message.sender ?? "" },
        });
        const { sessionId, store } = resolveSessionForEmergencyStop(route.sessionKey);
        emergencyStop({ cfg, sessionKey: route.sessionKey, sessionId, sessionStore: store });
        logVerbose(`imessage: emergency-stop fast-path fired for ${route.sessionKey}`);
        return;
      }
    }
