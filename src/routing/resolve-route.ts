...
function listAgents(cfg: IdleHandsConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: IdleHandsConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: IdleHandsConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<IdleHandsConfig, EvaluatedBindingsCache>();
