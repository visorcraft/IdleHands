...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.idlehandsCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.idlehandsCanvasA2UIAction &&
        typeof window.idlehandsCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.idlehandsSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("idlehands:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android IdleHands node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.idlehandsSendUserAction === "function"
        ? window.idlehandsSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.IDLEHANDS_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.IDLEHANDS_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>IdleHands Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
