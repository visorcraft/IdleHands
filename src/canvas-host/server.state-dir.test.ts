...
describe("canvas host state dir defaults", () => {
  it("uses IDLEHANDS_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("idlehands-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("IdleHands Canvas");
      } finally {
        await handler.close();
      }
    });
