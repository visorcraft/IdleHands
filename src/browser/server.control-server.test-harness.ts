...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.IDLEHANDS_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/idlehands" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "idlehands",
        profiles: {
          idlehands: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchIdleHandsChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.IDLEHANDS_GATEWAY_PORT;
  process.env.IDLEHANDS_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.IDLEHANDS_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.IDLEHANDS_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.IDLEHANDS_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.IDLEHANDS_GATEWAY_PASSWORD;
  } else {
    process.env.IDLEHANDS_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
