...
vi.mock("./chrome.js", () => ({
  resolveIdleHandsUserDataDir: vi.fn(() => "/tmp/idlehands-test/idlehands/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveIdleHandsUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "idlehands-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveIdleHandsUserDataDir).mockReturnValue(userDataDir);
