import type { BrowserConfig, BrowserProfileConfig, IdleHandsConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_IDLEHANDS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "idlehands" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_IDLEHANDS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "idlehands" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_IDLEHANDS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_IDLEHANDS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an IdleHands browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for idlehands/idlehands CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: IdleHandsConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_IDLEHANDS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "idlehands";
