  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          idlehands: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.IDLEHANDS_GATEWAY_PORT;
    process.env.IDLEHANDS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.IDLEHANDS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.IDLEHANDS_GATEWAY_PASSWORD;
