...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredIdleHandsTmpDir: vi.fn(() => "/tmp/idlehands"),
}));
vi.mock("../infra/tmp-idlehands-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredIdleHandsTmpDir.mockReturnValue("/tmp/idlehands");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredIdleHandsTmpDir.mockReturnValue("/tmp/idlehands-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "idlehands-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredIdleHandsTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "idlehands-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "idlehands-preferred", "downloads")}${path.sep}`),
    );
  });
