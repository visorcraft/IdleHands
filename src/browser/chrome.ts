...
export function resolveIdleHandsUserDataDir(profileName = DEFAULT_IDLEHANDS_BROWSER_PROFILE_NAME) {
...
export async function launchIdleHandsChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_IDLEHANDS_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateIdleHandsProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`idlehands browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopIdleHandsChrome(running: RunningChrome, timeoutMs = 2500) {
