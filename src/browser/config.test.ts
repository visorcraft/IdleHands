...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and hand-orange color", () => {
    ...
    const idlehands = resolveProfile(resolved, "idlehands");
    expect(idlehands?.driver).toBe("idlehands");
    expect(idlehands?.cdpPort).toBe(18800);
    expect(idlehands?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from IDLEHANDS_GATEWAY_PORT when unset", () => {
    withEnv({ IDLEHANDS_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const idlehands = resolveProfile(resolved, "idlehands");
      expect(idlehands?.cdpPort).toBe(19012);
      expect(idlehands?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ IDLEHANDS_GATEWAY_PORT: undefined }, () => {
      ...
      const idlehands = resolveProfile(resolved, "idlehands");
      expect(idlehands?.cdpPort).toBe(19022);
      expect(idlehands?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "idlehands");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        idlehands: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("idlehands");
  });
