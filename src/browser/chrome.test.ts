...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "idlehands-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateIdleHandsProfile(userDataDir, { color: DEFAULT_IDLEHANDS_BROWSER_COLOR });
    ...
    expect(def.shortcut_name).toBe(DEFAULT_IDLEHANDS_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".idlehands-profile-decorated"),
      "utf-8",
    );
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_IDLEHANDS_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopIdleHandsChrome no-ops when process is already killed", async () => {
    const proc = makeProc({ killed: true });
    await stopIdleHandsChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopIdleHandsChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopIdleHandsChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopIdleHandsChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
