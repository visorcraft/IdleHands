...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".idlehands-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + hand-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateIdleHandsProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_IDLEHANDS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_IDLEHANDS_BROWSER_COLOR).toUpperCase();
