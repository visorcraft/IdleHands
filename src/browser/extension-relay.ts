...
const RELAY_AUTH_HEADER = "x-idlehands-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/IdleHands-Extension-Relay",
            revision: "0",
            userAgent: "IdleHands-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "IdleHands/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedIdleHandsRelay({
