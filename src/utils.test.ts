...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("idlehands-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("idlehands-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("idlehands-lid-a-", (first) => {
      withTempDirSync("idlehands-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.idlehands when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "idlehands-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers IDLEHANDS_HOME over HOME", () => {
    vi.stubEnv("IDLEHANDS_HOME", "/srv/idlehands-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/idlehands-home"));
...
describe("shortenHomePath", () => {
  it("uses $IDLEHANDS_HOME prefix when IDLEHANDS_HOME is set", () => {
    vi.stubEnv("IDLEHANDS_HOME", "/srv/idlehands-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/idlehands-home")}/.idlehands/idlehands.json`)).toBe(
      "$IDLEHANDS_HOME/.idlehands/idlehands.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $IDLEHANDS_HOME replacement when IDLEHANDS_HOME is set", () => {
    vi.stubEnv("IDLEHANDS_HOME", "/srv/idlehands-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/idlehands-home")}/.idlehands/idlehands.json`),
    ).toBe("config: $IDLEHANDS_HOME/.idlehands/idlehands.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/idlehands")).toBe(path.resolve(os.homedir(), "idlehands"));
  });
  ...
  it("prefers IDLEHANDS_HOME for tilde expansion", () => {
    vi.stubEnv("IDLEHANDS_HOME", "/srv/idlehands-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/idlehands")).toBe(path.resolve("/srv/idlehands-home", "idlehands"));
