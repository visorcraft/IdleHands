...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "idlehands-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    IDLEHANDS_GATEWAY_TOKEN: "test-token",
    IDLEHANDS_CONFIG_DIR: join(sandbox.rootDir, "config"),
    IDLEHANDS_WORKSPACE_DIR: join(sandbox.rootDir, "idlehands"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      IDLEHANDS_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      IDLEHANDS_EXTRA_MOUNTS: undefined,
      IDLEHANDS_HOME_VOLUME: "idlehands-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("IDLEHANDS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("IDLEHANDS_EXTRA_MOUNTS=");
    expect(envFile).toContain("IDLEHANDS_HOME_VOLUME=idlehands-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("idlehands-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("idlehands-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg IDLEHANDS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      IDLEHANDS_CONFIG_DIR: configDir,
      IDLEHANDS_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when IDLEHANDS_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("IDLEHANDS_GATEWAY_TOKEN=config-token-123");
  });

  it("rejects injected multiline IDLEHANDS_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      IDLEHANDS_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("IDLEHANDS_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid IDLEHANDS_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      IDLEHANDS_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid IDLEHANDS_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      IDLEHANDS_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("IDLEHANDS_HOME_VOLUME must match");
  });
