...
const SEARCH_TOOL = "https://docs.idlehands.ai/mcp.SearchIdleHands";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.idlehands.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('idlehands docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.idlehands.ai/");
      runtime.log(`Search: ${formatCliCommand('idlehands docs "your query"')}`);
    }
    return;
  }
