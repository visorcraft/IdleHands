...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveIdleHandsAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("idlehands models auth setup-token")}\` or \`${formatCliCommand("idlehands configure")}\`.`
          : `Run \`${formatCliCommand("idlehands configure")}\` or set an API key env var.`;
