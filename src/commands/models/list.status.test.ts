...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveIdleHandsAgentDir: vi.fn().mockReturnValue("/tmp/idlehands-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/idlehands-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/idlehands-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveIdleHandsAgentDir: mocks.resolveIdleHandsAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/idlehands-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveIdleHandsAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/idlehands-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/idlehands-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/idlehands-agent-custom");
