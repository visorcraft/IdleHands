...
export async function loadValidConfigOrThrow(): Promise<IdleHandsConfig> {
...
export async function updateConfig(
  mutator: (cfg: IdleHandsConfig) => IdleHandsConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: IdleHandsConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: IdleHandsConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("idlehands agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
