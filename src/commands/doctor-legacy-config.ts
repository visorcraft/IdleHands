...
export function normalizeCompatibilityConfigValues(cfg: IdleHandsConfig): {
  config: IdleHandsConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: IdleHandsConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as IdleHandsConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as IdleHandsConfig["browser"],
    };
    changes.push(
