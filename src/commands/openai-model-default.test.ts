...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: IdleHandsConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: IdleHandsConfig) => { changed: boolean; next: IdleHandsConfig };
  defaultModel: string;
  overrideConfig: IdleHandsConfig;
  alreadyDefaultConfig: IdleHandsConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: IdleHandsConfig) => config,
      applyDefaultConfig: (config: IdleHandsConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: IdleHandsConfig = { agents: { defaults: {} } };
