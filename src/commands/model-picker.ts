...
type PromptDefaultModelResult = { model?: string; config?: IdleHandsConfig };
...
function resolveConfiguredModelRaw(cfg: IdleHandsConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: IdleHandsConfig): string[] {
...
export function applyPrimaryModel(cfg: IdleHandsConfig, model: string): IdleHandsConfig {
...
export function applyModelAllowlist(cfg: IdleHandsConfig, models: string[]): IdleHandsConfig {
  const defaults = cfg.agents?.defaults;
