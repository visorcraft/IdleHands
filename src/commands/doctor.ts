...
function resolveMode(cfg: IdleHandsConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("IdleHands doctor");

  const root = await resolveIdleHandsPackageRoot({
  ...
  let cfg: IdleHandsConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("idlehands configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("idlehands config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("idlehands setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("idlehands doctor --fix")}" to apply changes.`);
  }
