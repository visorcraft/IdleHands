...
export const resolveIdleHandsPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadIdleHandsPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/idlehands.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveIdleHandsPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.IDLEHANDS_STATE_DIR;
  originalUpdateInProgress = process.env.IDLEHANDS_UPDATE_IN_PROGRESS;
  process.env.IDLEHANDS_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "idlehands-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.IDLEHANDS_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.IDLEHANDS_UPDATE_IN_PROGRESS;
  } else {
    process.env.IDLEHANDS_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
