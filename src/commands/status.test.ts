...
beforeAll(() => {
  envSnapshot = captureEnv(["IDLEHANDS_PROFILE"]);
  process.env.IDLEHANDS_PROFILE = "isolated";
});
...
vi.mock("../infra/idlehands-root.js", () => ({
  resolveIdleHandsPackageRoot: vi.fn().mockResolvedValue("/tmp/idlehands"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/idlehands/pnpm-lock.yaml",
      markerPath: "/tmp/idlehands/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.idlehands.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.idlehands.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("IdleHands status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("idlehands status --all") ||
          l.includes("idlehands --profile isolated status --all") ||
          l.includes("idlehands status --all") ||
          l.includes("idlehands --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.IDLEHANDS_GATEWAY_TOKEN = "abcd1234";
    try {
