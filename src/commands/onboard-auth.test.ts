...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under IDLEHANDS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("idlehands-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-oauth-sync-"));
    ...
    process.env.IDLEHANDS_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-oauth-nosync-"));
    process.env.IDLEHANDS_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside IDLEHANDS_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-oauth-external-"));
    process.env.IDLEHANDS_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* IDLEHANDS_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to IDLEHANDS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("idlehands-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
