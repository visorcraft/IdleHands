...
type EnvSnapshot = {
  HOME?: string;
  IDLEHANDS_HOME?: string;
  IDLEHANDS_STATE_DIR?: string;
  IDLEHANDS_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    IDLEHANDS_HOME: process.env.IDLEHANDS_HOME,
    IDLEHANDS_STATE_DIR: process.env.IDLEHANDS_STATE_DIR,
    IDLEHANDS_OAUTH_DIR: process.env.IDLEHANDS_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: IdleHandsConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: IdleHandsConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "idlehands-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.IDLEHANDS_HOME = tempHome;
    process.env.IDLEHANDS_STATE_DIR = path.join(tempHome, ".idlehands");
    delete process.env.IDLEHANDS_OAUTH_DIR;
    fs.mkdirSync(process.env.IDLEHANDS_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when IDLEHANDS_OAUTH_DIR is explicitly configured", async () => {
    process.env.IDLEHANDS_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: IdleHandsConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints idlehands-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: IdleHandsConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/idlehands sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/idlehands sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /idlehands sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
