...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.IDLEHANDS_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.IDLEHANDS_GATEWAY_PASSWORD;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from idlehands.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "IdleHands update wizard" : "IdleHands configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: IdleHandsConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("idlehands doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
