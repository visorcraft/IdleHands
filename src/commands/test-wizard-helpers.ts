...
export async function setupAuthTestEnv(
  ...
  process.env.IDLEHANDS_AGENT_DIR = agentDir;
...
export function requireIdleHandsAgentDir(): string {
  const agentDir = process.env.IDLEHANDS_AGENT_DIR;
  if (!agentDir) {
    throw new Error("IDLEHANDS_AGENT_DIR not set");
  }
  return agentDir;
}
