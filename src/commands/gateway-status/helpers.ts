...
import type { IdleHandsConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: IdleHandsConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.IDLEHANDS_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.IDLEHANDS_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: IdleHandsConfig) {
