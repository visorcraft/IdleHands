...
export async function statusCommand(
  ...
  runtime.log(theme.heading("IdleHands status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`idlehands devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("idlehands devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("idlehands devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("idlehands security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("idlehands security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.idlehands.ai/faq");
  runtime.log("Troubleshooting: https://docs.idlehands.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("idlehands status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("idlehands logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("idlehands status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("idlehands gateway probe")}`);
  }
}
