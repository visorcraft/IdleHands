 ...
 * Runs as part of `idlehands doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("idlehands config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("idlehands memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("idlehands configure --section model")}`,
        `- To disable: ${formatCliCommand("idlehands config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("idlehands memory status --deep")}`,
      ].join("\n"),
      "Memory search",
