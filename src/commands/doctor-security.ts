...
import type { IdleHandsConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: IdleHandsConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("idlehands security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.idlehands/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("idlehands approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.idlehands.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("idlehands configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("idlehands config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("idlehands doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("idlehands config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('idlehands config set session.dmScope "per-channel-peer"') +
