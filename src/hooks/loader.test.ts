...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "idlehands-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["IDLEHANDS_BUNDLED_HOOKS_DIR"]);
    process.env.IDLEHANDS_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
