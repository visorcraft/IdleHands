...
homepage: https://docs.idlehands.ai/automation/hooks#command-logger
metadata:
  {
    "idlehands":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.idlehands/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.idlehands/logs/commands.log ~/.idlehands/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/idlehands`:
   ```
   /home/username/.idlehands/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
