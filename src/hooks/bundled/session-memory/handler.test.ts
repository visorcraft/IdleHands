...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => IdleHandsConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("idlehands-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): IdleHandsConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
