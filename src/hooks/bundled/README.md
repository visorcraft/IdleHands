...
This directory contains hooks that ship with IdleHands. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.idlehands/idlehands.json`:
...
2. Restart gateway: `pkill -9 -f 'idlehands.*gateway' && pnpm idlehands gateway`
3. Enable the hook: `idlehands hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.idlehands.ai/automation/hooks
