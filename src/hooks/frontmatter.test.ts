...
`;
    ...
    expect(parsed.idlehands.emoji).toBe("💾");
    expect(parsed.idlehands.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.idlehands.emoji).toBe("📝");
    expect(parsed.idlehands.events).toEqual(["command"]);
    expect(parsed.idlehands.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.idlehands.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"idlehands": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"idlehands": {"events": ["test"]}}');
  });
...
describe("resolveIdleHandsMetadata", () => {
  it("extracts idlehands metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveIdleHandsMetadata(frontmatter);
  ...
  it("returns undefined when idlehands key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        idlehands: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with IdleHands" },
            { id: "npm", kind: "npm", package: "@idlehands/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@idlehands/hook");
  });
...
`;
    ...
    const idlehands = resolveIdleHandsMetadata(frontmatter);
    expect(idlehands).toBeDefined();
    expect(idlehands?.emoji).toBe("💾");
    expect(idlehands?.events).toEqual(["command:new", "command:reset"]);
    expect(idlehands?.requires?.config).toEqual(["workspace.dir"]);
    expect(idlehands?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const idlehands = resolveIdleHandsMetadata(frontmatter);
    expect(idlehands?.emoji).toBe("disk");
    expect(idlehands?.events).toEqual(["command:new"]);
  });
});
