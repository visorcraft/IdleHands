...
function validateCommandTemplate(
  ...
  while ((m = re.exec(s)) !== null) {
    if (!ALLOWED_TEMPLATE_VARS.has(m[1]))
      throw new Error(`${where}: unknown template variable {${m[1]}}`);
  }
  return s;
}
...
function validateHost(raw: unknown, index: number): RuntimeHost {
  ...
  if (transport !== 'local' && transport !== 'ssh')
    throw new Error(`hosts[${index}].transport: expected "local" or "ssh"`);
  ...
  const connection = {
    host:
      raw.connection.host == null
        ? undefined
        : assertString(raw.connection.host, `hosts[${index}].connection.host`),
    port:
      raw.connection.port == null
        ? undefined
        : assertNumber(raw.connection.port, `hosts[${index}].connection.port`),
    user:
      raw.connection.user == null
        ? undefined
        : assertString(raw.connection.user, `hosts[${index}].connection.user`),
    key_path:
      raw.connection.key_path == null
        ? undefined
        : assertString(raw.connection.key_path, `hosts[${index}].connection.key_path`),
    password:
      raw.connection.password == null
        ? undefined
        : assertString(raw.connection.password, `hosts[${index}].connection.password`),
  };

  if (!isRecord(raw.capabilities)) throw new Error(`hosts[${index}].capabilities: expected object`);
  assertNoUnknownKeys(
    raw.capabilities,
    ['gpu', 'vram_gb', 'backends'],
    `hosts[${index}].capabilities`
  );
  const capabilities = {
    gpu: validateStringArray(raw.capabilities.gpu, `hosts[${index}].capabilities.gpu`),
    vram_gb:
      raw.capabilities.vram_gb == null
        ? undefined
        : assertNumber(raw.capabilities.vram_gb, `hosts[${index}].capabilities.vram_gb`),
    backends: validateStringArray(
      raw.capabilities.backends,
      `hosts[${index}].capabilities.backends`
    ),
  };
  ...
  const health = {
    check_cmd: validateCommandTemplate(
      raw.health.check_cmd,
      `hosts[${index}].health.check_cmd`
    ) as string,
    timeout_sec:
      raw.health.timeout_sec == null
        ? undefined
        : assertNumber(raw.health.timeout_sec, `hosts[${index}].health.timeout_sec`),
  };

  if (!isRecord(raw.model_control))
    throw new Error(`hosts[${index}].model_control: expected object`);
  assertNoUnknownKeys(
    raw.model_control,
    ['stop_cmd', 'cleanup_cmd'],
    `hosts[${index}].model_control`
  );
  const model_control = {
    stop_cmd: validateCommandTemplate(
      raw.model_control.stop_cmd,
      `hosts[${index}].model_control.stop_cmd`
    ) as string,
    cleanup_cmd: validateCommandTemplate(
      raw.model_control.cleanup_cmd,
      `hosts[${index}].model_control.cleanup_cmd`,
      true
    ),
  };
...
function validateBackend(raw: unknown, index: number): RuntimeBackend {
  ...
  if (!['vulkan', 'rocm', 'cuda', 'metal', 'cpu', 'custom'].includes(type))
    throw new Error(`backends[${index}].type: invalid backend type`);
  ...
  if (raw.env != null) {
    ...
    for (const [k, v] of Object.entries(raw.env))
      env[k] = assertString(v, `backends[${index}].env.${k}`);
  }
  ...
  return {
    ...
    verify_always:
      raw.verify_always == null
        ? undefined
        : assertBoolean(raw.verify_always, `backends[${index}].verify_always`),
    rollback_cmd: validateCommandTemplate(
      raw.rollback_cmd,
      `backends[${index}].rollback_cmd`,
      true
    ),
    env,
...
function validateModel(raw: unknown, index: number): RuntimeModel {
  ...
  assertNoUnknownKeys(
    raw.launch,
    ['start_cmd', 'probe_cmd', 'probe_timeout_sec', 'probe_interval_ms'],
    `models[${index}].launch`
  );
  const launch = {
    start_cmd: validateCommandTemplate(
      raw.launch.start_cmd,
      `models[${index}].launch.start_cmd`
    ) as string,
    probe_cmd: validateCommandTemplate(
      raw.launch.probe_cmd,
      `models[${index}].launch.probe_cmd`
    ) as string,
    probe_timeout_sec:
      raw.launch.probe_timeout_sec == null
        ? undefined
        : assertNumber(raw.launch.probe_timeout_sec, `models[${index}].launch.probe_timeout_sec`),
    probe_interval_ms:
      raw.launch.probe_interval_ms == null
        ? undefined
        : assertNumber(raw.launch.probe_interval_ms, `models[${index}].launch.probe_interval_ms`),
  };

  let runtime_defaults;
  if (raw.runtime_defaults != null) {
    if (!isRecord(raw.runtime_defaults))
      throw new Error(`models[${index}].runtime_defaults: expected object`);
    assertNoUnknownKeys(
      raw.runtime_defaults,
      ['port', 'context_window', 'max_tokens'],
      `models[${index}].runtime_defaults`
    );
    runtime_defaults = {
      port:
        raw.runtime_defaults.port == null
          ? undefined
          : assertNumber(raw.runtime_defaults.port, `models[${index}].runtime_defaults.port`),
      context_window:
        raw.runtime_defaults.context_window == null
          ? undefined
          : assertNumber(
              raw.runtime_defaults.context_window,
              `models[${index}].runtime_defaults.context_window`
            ),
      max_tokens:
        raw.runtime_defaults.max_tokens == null
          ? undefined
          : assertNumber(
              raw.runtime_defaults.max_tokens,
              `models[${index}].runtime_defaults.max_tokens`
            ),
    };
  }
...
export function validateRuntimes(input: unknown): RuntimesConfig {
  ...
  for (const key of Object.keys(input)) {
    if (!topAllowed.includes(key))
      console.error(`[runtimes] warning: unknown top-level key "${key}"`);
  }
  ...
  if (hasSecretRefStub) {
    console.error(
      '[runtime] warning: Secret refs (*_ref fields) are not yet supported. Use inline values for now.'
    );
  }
    ...
    if (backend.host_filters !== 'any') {
      for (const hostId of backend.host_filters) {
        if (!hostIds.has(hostId))
          throw new Error(`backends[${backend.id}].host_filters: unknown host id "${hostId}"`);
      }
    }
  }
  ...
  for (const model of models) {
    if (model.host_policy !== 'any') {
      for (const hostId of model.host_policy) {
        if (!hostIds.has(hostId))
          throw new Error(`models[${model.id}].host_policy: unknown host id "${hostId}"`);
      }
    }
    if (model.backend_policy !== 'any') {
      for (const backendId of model.backend_policy) {
        if (!backendIds.has(backendId))
          throw new Error(`models[${model.id}].backend_policy: unknown backend id "${backendId}"`);
      }
    }
  }
...
export async function loadRuntimes(filePath = runtimesPath()): Promise<RuntimesConfig> {
  ...
  const parsed = raw.trim()
    ? JSON.parse(raw)
    : { schema_version: 1, hosts: [], backends: [], models: [] };
  return validateRuntimes(parsed);
}

export async function saveRuntimes(
  config: RuntimesConfig,
  filePath = runtimesPath()
): Promise<void> {
