 ...
 * - Movement: h, l, w, b, 0, $, ^
 * - Editing: x, dd, i, a, A, I, o (appends newline placeholder)
 * - Yank/Paste: yy, p
...
export function handleVimKeypress(
  ...
  if (k === 'x') {
    if (cursor < line.length) {
      ...
      setLine(rl, newLine);
      setCursor(rl, Math.min(cursor, newLine.length));
    }
    return true;
  }
  ...
  if (k === 'p') {
    if (state.yankBuffer) {
      ...
      setLine(rl, newLine);
      setCursor(rl, cursor + state.yankBuffer.length);
    }
    return true;
  }
...
function setLine(rl: readline.Interface, text: string): void {
