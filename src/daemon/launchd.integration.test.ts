...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `idlehands-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      IDLEHANDS_LAUNCHD_LABEL: `ai.idlehands.launchd-int-${testId}`,
      IDLEHANDS_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
