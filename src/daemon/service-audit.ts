...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.IDLEHANDS_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service IDLEHANDS_GATEWAY_TOKEN does not match gateway.auth.token in idlehands.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `idlehands gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
