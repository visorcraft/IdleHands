...
export const DEFAULT_LOG_DIR = resolvePreferredIdleHandsTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "idlehands.log"); // legacy single-file path

const LOG_PREFIX = "idlehands";
...
function resolveSettings(): ResolvedSettings {
  let cfg: IdleHandsConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => IdleHandsConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.IDLEHANDS_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[idlehands] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
