...
describe("resolveGatewayConnection", () => {
  ...
  it("uses IDLEHANDS_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ IDLEHANDS_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers IDLEHANDS_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ IDLEHANDS_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
