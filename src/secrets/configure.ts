...
function getSecretProviders(config: IdleHandsConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: IdleHandsConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: IdleHandsConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: IdleHandsConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: IdleHandsConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: IdleHandsConfig; next: IdleHandsConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in idlehands.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "idlehands secrets configure",
