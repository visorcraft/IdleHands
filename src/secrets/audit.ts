...
import { resolveStateDir, type IdleHandsConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: IdleHandsConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.IDLEHANDS_AUTH_STORE_READONLY;
  process.env.IDLEHANDS_AUTH_STORE_READONLY = "1";
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as IdleHandsConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.IDLEHANDS_AUTH_STORE_READONLY;
    } else {
      process.env.IDLEHANDS_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
