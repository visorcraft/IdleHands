...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ IDLEHANDS_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.IDLEHANDS_BASE_ENV ?? "") + "|" + (process.env.IDLEHANDS_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { IDLEHANDS_TEST_ENV: "ok" },
        },
      );
