...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/idlehands/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/idlehands");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/idlehands/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__IDLEHANDS_CONTROL_UI_BASE_PATH__ = "/idlehands";
    const app = mountApp("/idlehands/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/idlehands");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/idlehands/sessions");
  });
