...
describe('Anton git functions', () => {
  ...
  test('createBranch throws on duplicate branch name', () => {
    ...
    try {
      ...
      const origBranch = execSync('git rev-parse --abbrev-ref HEAD', {
        cwd: tmpDir,
        encoding: 'utf8',
      }).trim();
      ...
      assert.throws(() => createBranch(tmpDir, 'test-branch'), /git checkout -b failed/);
    } finally {
      cleanup(tmpDir);
    }
  });
  ...
  test('commitAmend amends last commit without changing message', () => {
    ...
    try {
      ...
      const files = execSync('git show --name-only --pretty=format:', {
        cwd: tmpDir,
        encoding: 'utf8',
      })
        .trim()
