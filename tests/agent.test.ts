...
import type { ToolCallEvent, ToolResultEvent, TurnEndEvent } from '../src/types.js';
...
describe('agent loop dispatch + hooks', () => {
  it('dispatches tool calls and emits hook lifecycle events', async () => {
    ...
    const out = await session.ask('list files', {
      onToolCall: (ev: ToolCallEvent) => calls.push(ev),
      onToolResult: (ev: ToolResultEvent) => results.push(ev),
      onTurnEnd: (ev: TurnEndEvent) => turns.push(ev),
    });
