...
`;
      ...
      // Test idempotent
      await appendTaskNote(testFile, taskKey, 'Test note');
      const contentAfter = await import('node:fs/promises').then((fs) =>
        fs.readFile(testFile, 'utf8')
      );
...
`;
  ...
  test('16. findRunnablePendingTasks → parent with pending child: child runs, parent skipped', async () => {
    const content = `- [ ] Parent task
  - [ ] Child task
- [ ] Independent task
`;
    ...
    assert.equal(runnable[0].text, 'Child task');
    assert.equal(runnable[1].text, 'Independent task');
    // Parent delegates to child — parent is NOT runnable
  });

  test('16b. findRunnablePendingTasks → parent with all children checked IS runnable', async () => {
    const content = `- [ ] Parent task
  - [x] Child task 1
  - [x] Child task 2
- [ ] Independent task
`;
  ...
  test('16c. findRunnablePendingTasks → deeply nested: grandchildren run when ancestors delegate', async () => {
...
`;
    ...
    assert.equal(runnable[0].text, 'Grandchild 1');
    assert.equal(runnable[1].text, 'Grandchild 2');
    assert.equal(runnable[2].text, 'Independent task');
  });
