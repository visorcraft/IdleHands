...
describe('buildAntonPrompt', () => {
  it('should contain basic task information and rules', async () => {
    ...
    assert.ok(prompt.includes('You are an autonomous coding agent working on exactly ONE task'));
    assert.ok(prompt.includes('Complete ONLY the task described below'));
    assert.ok(prompt.includes('Do NOT modify or check off tasks in the task file'));
    assert.ok(prompt.includes('Keep changes minimal, focused, and scoped'));
