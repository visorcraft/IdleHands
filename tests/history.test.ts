...
import { stripThinking, estimateTokensFromMessages, enforceContextBudget, estimateToolSchemaTokens } from '../dist/history.js';
...
describe('estimateToolSchemaTokens', () => {
  ...
  it('scales with tool count', () => {
    ...
    const largeSet = Array.from({ length: 10 }, (_, i) => ({
      type: 'function', function: { name: `tool_${i}`, description: 'A tool that does things with parameters and descriptions that add tokens', parameters: { type: 'object', properties: { arg: { type: 'string' } } } }
    }));
...
describe('enforceContextBudget', () => {
  it('keeps messages when under budget', () => {
    ...
    const result = enforceContextBudget({ messages: msgs, contextWindow: 131072, maxTokens: 16384 });
  ...
  it('drops tool-result messages before user/assistant messages', () => {
    const msgs: any[] = [
      ...
      { role: 'assistant', content: 'a3', tool_calls: [{ id: 'c2', type: 'function', function: { name: 'read_file', arguments: '{}' } }] },
    ...
    const result = enforceContextBudget({ messages: msgs, contextWindow: 8192, maxTokens: 2048, minTailMessages: 4 });
    ...
    assert.ok(toolMsgs.length <= userMsgs.length, `tool msgs (${toolMsgs.length}) should be <= user msgs (${userMsgs.length})`);
  });
  ...
  it('drops assistant+tool groups together (no orphaned tool_calls)', () => {
    ...
    const result = enforceContextBudget({ messages: msgs, contextWindow: 8192, maxTokens: 2048, minTailMessages: 3 });
    ...
    const hasAssistantWithToolCalls = result.some((m: any) => m.role === 'assistant' && m.tool_calls?.length);
    // They must either both be present or both be absent.
    assert.equal(hasToolResult, hasAssistantWithToolCalls,
  ...
  it('triggers compaction at 80% of budget', () => {
    ...
    assert.ok(result.length < msgs.length, `Should have compacted: ${result.length} < ${msgs.length}`);
  });
  ...
  it('toolSchemaTokens tightens the budget', () => {
    ...
    const withDefault = enforceContextBudget({ messages: msgs, contextWindow: 131072, maxTokens: 16384 });
    const withLargeSchema = enforceContextBudget({ messages: msgs, contextWindow: 131072, maxTokens: 16384, toolSchemaTokens: 5000 });
    // Larger tool schema overhead means tighter budget → more aggressive compaction
    assert.ok(withLargeSchema.length <= withDefault.length,
