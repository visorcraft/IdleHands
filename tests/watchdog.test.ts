...
describe('watchdog helpers', () => {
  it('resolves defaults and clamps values', () => {
    ...
    assert.equal(cfg.clamped, true, 'should flag clamped when timeout was raised to floor');
  });
  ...
  it('uses primary values over fallback values', () => {
    ...
    assert.equal(cfg.clamped, false, 'should not flag clamped for normal values');
  });
  ...
  it('formatWatchdogCancelMessage handles Error objects', () => {
    const msg = formatWatchdogCancelMessage({
      watchdogForcedCancel: false,
      maxCompactions: 3,
      debugAbortReason: true,
      abortReason: new Error('AbortError: signal was aborted'),
    });
    assert.ok(msg.includes('[debug] AbortError: signal was aborted'));
    assert.ok(!msg.includes('[object Object]'), 'should not stringify Error to [object Object]');
  });

  it('formatWatchdogCancelMessage truncates long reasons to 400 chars', () => {
    ...
    // The reason portion should be truncated
    const debugPart = msg.split('[debug] ')[1];
    assert.equal(debugPart.length, 400);
  });
});
