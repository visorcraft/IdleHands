---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for IdleHands deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, IdleHands cron scheduling for periodic checks, or version status checks on a machine running IdleHands (laptop, workstation, Pi, VPS).
---

# IdleHands Host Hardening

## Overview

Assess and harden the host running IdleHands, then align it to a user-defined risk tolerance without breaking access. Use IdleHands security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim IdleHands changes the host firewall, SSH, or OS updates; it does not.
...
5. IdleHands gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if IdleHands can access sensitive data.
...
### 2) Run IdleHands security audits (read-only)

As part of the default read-only checks, run `idlehands security audit --deep`. Only offer alternatives if the user requests them:

1. `idlehands security audit` (faster, non-probing)
2. `idlehands security audit --json` (structured output)

Offer to apply IdleHands safe defaults (numbered):

1. `idlehands security audit --fix`

Be explicit that `--fix` only tightens IdleHands defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check IdleHands version/update status (read-only)

As part of the default read-only checks, run `idlehands update status`.
...
- Credential hygiene notes (location of IdleHands creds, prefer disk encryption)
...
- IdleHands security audit (re-run)
...
After IdleHands install or first hardening pass, run at least one baseline audit and version check:

- `idlehands security audit`
- `idlehands security audit --deep`
- `idlehands update status`

Ongoing monitoring is recommended. Use the IdleHands cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `idlehands cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `idlehands update status`
...
Before creating, `idlehands cron list` and match on exact `name`. If found, `idlehands cron edit <id> ...`.
If not found, `idlehands cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `idlehands update status` (preferred for source checkouts and channels)
2. `npm view idlehands version` (published npm version)

## IdleHands command accuracy

Use only supported commands and flags:

- `idlehands security audit [--deep] [--fix] [--json]`
- `idlehands status` / `idlehands status --deep`
- `idlehands health --json`
- `idlehands update status`
- `idlehands cron add|list|runs|run`

Do not invent CLI flags or imply IdleHands enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by IdleHands compaction:
